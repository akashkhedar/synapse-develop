...
env_pid = os.getenv("SYNAPSE_PROJECT_ID")
project = None
if env_pid:
    project = client.projects.get(id=int(env_pid))
else:
...
client.projects.update(id=project.id, sampling="Uncertainty sampling")
...
labeled_tasks = [
    t.model_dump() for t in all_tasks_full if getattr(t, "annotations", None)
]
...
client.projects.update(id=project.id, model_version=model_version)
