"""
This is a Python script that exports labeled data from a Synapse project
and converts it into the YOLO format, including downloading associated images.

## 1. What It Does and How
The script performs the following steps:

1. **Connects to Synapse:** It uses the Synapse SDK to connect
to a Synapse instance using the provided URL and API key.
...
5. **Converts Annotations to YOLO Format:** Utilizes the Synapse Converter
...
1. **Install Dependencies:** Ensure you have Python installed along with the required packages:
   ```bash
   pip install git+https://github.com/synapse/synapse-sdk.git tqdm
   ```

2. **Set Up Credentials:** Provide your SYNAPSE_URL, SYNAPSE_API_KEY, and PROJECT_ID either
by modifying the script variables or via command-line arguments.

3. **Run the Script:** Execute the script from the terminal:
   ```bash
   python script.py --url 'https://app.Synapse.com' --api-key 'your-api-key' --project-id 12345
   ```
...
## 3. When to Use It
Use this script when you need to prepare labeled datasets from Synapse for training YOLO object detection models.
...
import os
from synapse_sdk.converter import Converter
...
def prepare_export(ls: Synapse, project_id: int, view_id: int | None = None):
...
def run(url: str, api_key: str, project_id: int):
    ...
    client = Synapse(base_url=url, api_key=api_key)
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="YOLO Export Script for Synapse")
    parser.add_argument(
        "--url",
        type=str,
        default=os.getenv("SYNAPSE_URL", SYNAPSE_URL),
        help="Synapse URL",
    )
    parser.add_argument(
        "--api-key",
        type=str,
        default=os.getenv("SYNAPSE_API_KEY", SYNAPSE_API_KEY),
        help="Synapse API Key",
    )
    parser.add_argument(
        "--project-id",
        type=int,
