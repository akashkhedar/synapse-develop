# Migrate from one Synapse instance to another

This migration helps to copy projects from one Synapse instance to another. To be copied:
...
```
pip install -U synapse-sdk
```

# How it works?

1. Get all projects from source Synapse (or specific projects)
2. Get all users from the organization
3. Create all users with the same emails, first names and last names on the target Synapse instance
4. For each project:

   4.1. export on the source Synapse instance, wait until export is done

   > Note: if there is an export error, skip this project

   4.2. create a new project on the target Synapse instance with the same params as on source instance, except review settings and assignment settings (enterprise only)
...
2. Go to source (src) and destination (dst) Synapse instances. Open Account pages (/user/account), copy your Access tokens.
...
3. Migrate projects from the organization:
   ...
   python3 migrate-ls-to-ls.py --project-ids=123,456 \
       ...
       --dst-url https://app.Synapse.com --dst-key <dst-token>
   ```
