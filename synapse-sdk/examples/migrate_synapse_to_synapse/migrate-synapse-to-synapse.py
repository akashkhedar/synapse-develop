"""Migrate LS to Synapse script.
This migration helps to copy projects from one Synapse instance to another.
...
import os
from synapse_sdk import Client
from synapse_sdk._legacy.users import User
from synapse_sdk.data_manager import Filters, Operator, Type, Column
...
class Migration:
    def __init__(self, src_url, src_key, dst_url, dst_key, dest_workspace):
        """Initialize migration that copy projects from one Synapse instance to another

        :param src_url: source Synapse instance
        :param src_key: source Synapse token
        :param dst_url: destination Synapse instance
        :param dst_key: destination Synapse token
        :param dest_workspace: destination workspace id
        """
        # Connect to the Synapse API and check the connection
        self.src_client = Client(url=src_url, api_key=src_key)
        self.dst_client = Client(url=dst_url, api_key=dst_key)
...
def run():
    ...
    parser = argparse.ArgumentParser(description="Synapse Project Migration Script")
    parser.add_argument(
        "--src-url",
        dest="src_url",
        type=str,
        default=os.getenv("SYNAPSE_URL", ""),
        help="Source Synapse instance (default from SYNAPSE_URL)",
    )
    parser.add_argument(
        "--src-key",
        dest="src_key",
        ...
        help="Source Synapse token (default from SYNAPSE_API_KEY)",
    )
    parser.add_argument(
        "--dst-url",
        dest="dst_url",
        type=str,
        default=os.getenv("DEST_SYNAPSE_URL", ""),
        help="Destination Synapse instance (default from DEST_SYNAPSE_URL)",
    )
    parser.add_argument(
        "--dst-key",
        dest="dst_key",
        type=str,
        default=os.getenv("DEST_SYNAPSE_API_KEY", ""),
        help="Destination Synapse token (default from DEST_SYNAPSE_API_KEY)",
    )
    parser.add_argument(
        "--project-ids",
        dest="project_ids",
