...
SYNAPSE_URL = os.getenv("SYNAPSE_URL", "https://app.heartex.com")
API_KEY = os.getenv("SYNAPSE_API_KEY", "<ls-api-key>")
...
def create_tags_with_random_distribution(
    ...
    response = ls._client_wrapper.httpx_client.request(
        method="post",
        path=f"/api/dm/actions?id=add_data_field&project={project_id}",
        json=data,
    )
...
def run():
    ...
    """
    project_id = int(os.getenv("SYNAPSE_PROJECT_ID", "12716"))
    reviewer_email = os.getenv("SYNAPSE_REVIEWER_EMAIL", "your@email.com")
