...
Install:
    ...
    cd synapse-sdk
    pip install -e .
    python examples/synapse_enterprise/assigner.py
...
class BatchAssigner:
    def __init__(self, host, api_key, project_id):
        self.client = Synapse(base_url=host, api_key=api_key)
        self.project = self.client.projects.get(id=project_id)
...
def start():
    host = os.getenv("SYNAPSE_URL", "http://localhost:8080")
    api_key = os.getenv("SYNAPSE_API_KEY", "")
    project_id = int(os.getenv("SYNAPSE_PROJECT_ID", "182"))

    filter_column = "shortname"
    filter_value = "opossum"
    page_size = 10
    emails = os.getenv(
        "SYNAPSE_ASSIGN_EMAILS", "user1@example.com,user2@example.com"
    ).split(",")
