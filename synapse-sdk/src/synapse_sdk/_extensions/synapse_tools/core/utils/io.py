...
from synapse_sdk._extensions.synapse_tools.core.utils.params import get_env
...
def get_local_path(
    ...
    :param hostname: Synapse Hostname, it will be used for uploaded files, local storage files and cloud storage files
      if not provided, it will be taken from SYNAPSE_URL env variable
    :param image_dir: Image and other media upload directory
    :param access_token: Synapse access token, it will be used for uploaded files, local storage files and cloud storage files
      if not provided, it will be taken from SYNAPSE_API_KEY env variable
    :param download_resources: Download and cache a file from URL
    :param task_id: Synapse Task ID, required for cloud storage files 
      because the URL will be rebuilt to `{hostname}/tasks/{task_id}/presign/?fileuri={url}` 

    :return: filepath
    """
    # get environment variables
    hostname = hostname or os.getenv("SYNAPSE_URL", "") or os.getenv("SYNAPSE_HOST", "")
    access_token = (
        access_token
        or os.getenv("SYNAPSE_API_KEY", "")
        or os.getenv("SYNAPSE_ACCESS_TOKEN", "")
    )
    if "localhost" in hostname:
        logger.warning(
            f"Using `localhost` ({hostname}) in SYNAPSE_URL, "
            f"`localhost` is not accessible inside of docker containers. "
            f"You can check your IP with utilities like `ifconfig` and set it as SYNAPSE_URL."
        )
    if hostname and not (
        hostname.startswith("http://") or hostname.startswith("https://")
    ):
        raise ValueError(
            f"Invalid hostname in SYNAPSE_URL: {hostname}. "
    ...
    # instead of downloading them from Synapse instance
    ...
    if is_uploaded_file or is_local_storage_file or is_cloud_storage_file:
        ...
        if not hostname:
            raise FileNotFoundError(
                ...
                "You can set SYNAPSE_URL environment variable to use it as a hostname."
            )
        ...
        # s3, gs, azure-blob file
            ...
            if task_id is None:
                raise Exception("Synapse Task ID is required for cloud storage files")
        ...
        if not access_token:
            raise FileNotFoundError(
                ...
                "set SYNAPSE_API_KEY environment variable."
            )
...
def download_and_cache(
    ...
    if not os.path.exists(filepath):
        ...
        if download_resources:
            ...
            # check if url matches hostname - then uses access token to this Synapse instance
            if (
                access_token
                and hostname
