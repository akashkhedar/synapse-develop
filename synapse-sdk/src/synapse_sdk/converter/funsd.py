"""This code allows to export Synapse Export JSON to FUNSD format.
It's only the basic converter, it converts every bbox as a separate word.
Check this github issue for more details:
https://github.com/synapse/synapse/issues/2634#issuecomment-1251648670

Usage: funsd.py export.json
This command will export your Synapse OCR annotations to "./funsd/" directory.
"""
...
def convert_annotation_to_fund(result):
    # collect all Synapse results and combine labels, text, coordinates into one record
...
def synapse_to_funsd_converter(
    synapse_export_path=\"export.json\", funsd_dir=\"funsd\", data_key=\"ocr\"
):
    with open(synapse_export_path) as f:
        tasks = json.load(f)
...
if __name__ == "__main__":
    ...
    print('This command will export your Synapse OCR annotations to \"./funsd/\" directory')

    synapse_to_funsd_converter(sys.argv[1])
