...
from synapse_sdk.converter.utils import ExpandFullPath
from synapse_sdk.converter.imports.label_config import generate_label_config

logger = logging.getLogger("root")


def new_task(out_type, root_url, file_name):
    """Create a new Synapse task structure.
    ...
    Returns:
        dict: Synapse task structure with image data and empty result array
    """
    return {
...
def create_segmentation(
    ...
    """Convert COCO segmentation annotation to Synapse polygon format.

    COCO segmentation format: flat array of [x1,y1,x2,y2,...] coordinates
    Synapse format: array of [x,y] points as percentages
    ...
    Returns:
        dict: Synapse polygon annotation item
    """
...
def create_keypoints(
    ...
    """Convert COCO keypoints annotation to Synapse keypoint format.
    ...
    Args:
        ...
        image_height (int): Height of the source image in pixels
        image_width (int): Width of the source image in pixels
        point_width (float): Width/size of keypoints in Synapse

    Returns:
        list: List of Synapse keypoint annotation items
    """
    label = categories[int(annotation["category_id"])]
...
def convert_coco_to_ls(
    ...
    """Convert COCO dataset annotations to Synapse JSON format.

    This function processes a COCO dataset JSON file and converts all annotations
    (bounding boxes, segmentations, keypoints) to Synapse's format. It supports:
    - Object detection (bounding boxes)
    - Instance segmentation (polygons)
    - Keypoint detection (pose estimation)

    The function automatically generates a Synapse labeling configuration
    based on the annotation types found in the COCO dataset.

    Args:
        input_file (str): Path to input COCO JSON file
        out_file (str): Path to output Synapse JSON file
        to_name (str, optional): Object name from Synapse labeling config. Defaults to "image".
        from_name (str, optional): Control tag name from Synapse labeling config. Defaults to "label".
    ...
    if len(tasks) > 0:
        ...
        logger.info("Saving Synapse JSON to %s", out_file)
...
def add_parser(subparsers):
    ...
    coco.add_argument(
        ...
        "--output",
        dest="output",
        help="output file with Synapse JSON tasks",
        default="output.json",
        action=ExpandFullPath,
    )
    coco.add_argument(
        "--to-name",
        dest="to_name",
        help="object name from Synapse labeling config",
        default="image",
    )
    coco.add_argument(
        "--from-name",
        dest="from_name",
        help="control tag name from Synapse labeling config",
        default="label",
    )
    coco.add_argument(
        "--out-type",
