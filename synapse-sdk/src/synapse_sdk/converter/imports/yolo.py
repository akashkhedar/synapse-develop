...
def convert_yolo_to_ls(
    ...
    """Convert YOLO labeling to Synapse JSON
    :param input_dir: directory with YOLO where images, labels, notes.json are located
    :param out_file: output file with Synapse JSON tasks
    :param to_name: object name from Synapse labeling config
    :param from_name: control tag name from Synapse labeling config
    ...
    for f in os.listdir(images_dir):
        ...
        if os.path.exists(label_file):
            ...
            with open(label_file) as file:
                # convert all bounding boxes to Synapse Results
    ...
    if len(tasks) > 0:
        ...
        if image_root_url == default_image_root_url:
            help_root_dir = (
                f"Set environment variables SYNAPSE_LOCAL_FILES_SERVING_ENABLED=true and "
                f"SYNAPSE_LOCAL_FILES_DOCUMENT_ROOT={input_dir} for Synapse run,\n"
        ...
        print(
            ...
            f"       See tutorial here:\nhttps://github.com/Synapse/synapse-converter/tree/master?tab=readme-ov-file#yolo-to-synapse-converter\n"
            f"       {help_root_dir}\n"
