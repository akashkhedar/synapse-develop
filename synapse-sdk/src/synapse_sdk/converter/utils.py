...
from synapse_sdk._extensions.synapse_tools.core.utils.io import safe_build_path
...
def _get_upload_dir(project_dir=None, upload_dir=None):
    """Return either upload_dir, or path by SYNAPSE_UPLOAD_DIR, or project_dir/upload"""
    if upload_dir:
        return upload_dir
    upload_dir = os.environ.get("SYNAPSE_UPLOAD_DIR")
    ...
    if not upload_dir:
        raise FileNotFoundError(
            "Can't find upload dir: either SYNAPSE_UPLOAD_DIR or project should be passed to converter"
        )
    return upload_dir
...
def convert_yolo_obb_to_annotation(xyxyxyxy, original_width, original_height):
    """
    Convert YOLO Oriented Bounding Box (OBB) format to Synapse format.
    ...
    # Create the dictionary for Synapse
    return {
        "x": x,
        "y": y,
        "width": width,
