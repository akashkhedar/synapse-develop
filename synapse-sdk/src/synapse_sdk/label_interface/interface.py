...
from synapse_sdk._legacy.exceptions import (
    LSConfigParseException,
    SynapseXMLSyntaxErrorSentryIgnored,
    SynapseValidationErrorSentryIgnored,
)

from .base import SynapseTag
...
file_path = os.path.join(
    dir_path, "..", "_legacy", "schema", "label_config_schema.json"
)
...
def _fix_choices(config):
    ...
    https://github.com/synapse/synapse/issues/1259
    """
...
class LabelInterface:
    ...
    within the Synapse ecosystem.

    It is designed to be compatible at the data structure level with
    an existing parser widely used within the Synapse ecosystem.
    ...
    ```python
    from synapse_sdk.label_interface import LabelInterface
    ...
    # returns a JSON representing a Synapse region
    region.as_json()
    ...
    @classmethod
    def create(
        cls, tags, mapping=None, title=None, style=None, pretty=True, *args, **kwargs
    ):
    ...
    def create_instance(cls, *args, **kwargs):
        ...
        Studio JSON format and uploaded to Synapse
    ...
    def create_regions(
        self, data: Dict[str, Union[str, Dict, List[str], List[Dict]]]
    ) -> List[Region]:
        """
    ...
    def ner_tags(self):
        return self.find_tags(
            "controls", lambda t: t.tag.lower() in ("labels", "hypertextlabels")
        )
    ...
    def parse(self, config_string: str) -> Tuple[Dict, Dict, Dict, etree._Element]:
        ...
        try:
            ...
            raise SynapseXMLSyntaxErrorSentryIgnored(str(e))
        ...
        for tag in xml_tree.iter():
            ...
            if ControlTag.validate_node(tag):
                controls[tag.attrib["name"]] = ControlTag.parse_node(
                    tag, tags_mapping=self._tags_mapping
                )

            elif ObjectTag.validate_node(tag):
                objects[tag.attrib["name"]] = ObjectTag.parse_node(
                    tag, tags_mapping=self._tags_mapping
                )
    ...
    def _schema_validation(self, config_string):
        ...
        try:
            ...
            raise SynapseValidationErrorSentryIgnored(str(exc))
            ...
            raise SynapseValidationErrorSentryIgnored(error_message)
        ...
        for toName_ in toNames:
            for toName in toName_.split(","):
                if toName not in names:
                    raise SynapseValidationErrorSentryIgnored(
    ...
    def is_valid(self):
        ...
        try:
            self.validate()
            return True
        except SynapseValidationErrorSentryIgnored:
            return False
        ...
        try:
                ...
                if score is not None:
                    try:
                        ...
                        if not (0.0 <= score_float <= 1.0):
                            errors.append(
                                f"Score must be between 0.00 and 1.00 inclusive, got {score_float}"
                            )
            ...
            for i, region in enumerate(result):
                if not isinstance(region, dict):
                    errors.append(
                        f"Region must be a dictionary. Got {type(region)}: {region}"
                    )
                    continue
            ...
            for i, relation in relations_to_validate:
                relation_error = self.validate_relation(
                    relation, regions, return_errors=True, relation_index=i
                )
                if relation_error:
    ...
    def _validate_region_logic(self, region, region_index=0) -> Tuple[bool, List[str]]:
        ...
        for field in required_fields:
            if field not in region:
                errors.append(
                    f"Region {region_index}: Missing required field '{field}'"
                )
        ...
        try:
            ...
            errors.append(
                f"Region {region_index}: 'from_name' '{region['from_name']}' not found in configuration"
            )
            return False, errors
        ...
        try:
            ...
            errors.append(
                f"Region {region_index}: 'to_name' '{region['to_name']}' not found in configuration"
            )
            return False, errors
        ...
        if actual_type != expected_type:
            errors.append(
                f"Region {region_index}: Type '{actual_type}' does not match expected type '{expected_type}' for control '{region['from_name']}'"
            )

        # Validate to_name is in control's to_name list
        if region["to_name"] not in control.to_name:
            errors.append(
                f"Region {region_index}: 'to_name' '{region['to_name']}' is not valid for control '{region['from_name']}'. Valid options: {control.to_name}"
            )
        ...
        try:
            ...
            errors.append(
                f"Region {region_index}: Error validating value for control '{region['from_name']}': {str(e)}"
            )
    ...
    def _validate_relation_logic(
        self, relation, regions, relation_index=0
    ) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            for field in required_fields:
                if field not in relation:
                    errors.append(
                        f"Relation {relation_index}: Missing required field '{field}'"
                    )

            # Validate type
            if relation.get("type") != "relation":
                errors.append(
                    f"Relation {relation_index}: Type must be 'relation', got '{relation.get('type')}'"
                )
            ...
            if relation.get("direction") not in valid_directions:
                errors.append(
                    f"Relation {relation_index}: Direction must be one of {valid_directions}, got '{relation.get('direction')}'"
                )
            ...
            if from_id not in region_mapping:
                errors.append(
                    f"Relation {relation_index}: 'from_id' '{from_id}' not found in regions"
                )
            ...
            if to_id not in region_mapping:
                errors.append(
                    f"Relation {relation_index}: 'to_id' '{to_id}' not found in regions"
                )

        except Exception as e:
            errors.append(
                f"Relation {relation_index}: Error validating relation - {str(e)}"
            )

        return len(errors) == 0, errors

    def validate_relation(
        self, relation, regions, return_errors=False, relation_index=0
    ):
        ...
        """
        is_valid, errors = self._validate_relation_logic(
            relation, regions, relation_index
        )
    ...
    def generate_sample_prediction(self) -> Optional[dict]:
        ...
        prediction = PredictionValue(
            model_version="sample model version", result=self._generate_sample_regions()
        )
        ...
        if self.validate_prediction(prediction_dct):
            ...
            logger.debug(
                f"Sample prediction {prediction_dct} failed validation for label config {self.config}"
            )
            return None
    ...
    def generate_sample_annotation(self) -> Optional[dict]:
        ...
        if self.validate_annotation(annotation_dct):
            ...
            logger.debug(
                f"Sample annotation {annotation_dct} failed validation for label config {self.config}"
            )
            return None

    def generate_complete_sample_task(
        ...
        if not sample_task:
            if raise_on_failure:
                raise ValueError(
                    "LabelInterface.generate_sample_task failed to generate a valid sample task"
                )
            return None
        ...
        if not sample_prediction and raise_on_failure:
            raise ValueError(
                "LabelInterface.generate_sample_prediction failed to generate a valid prediction"
            )

        sample_annotation = self.generate_sample_annotation()
        if not sample_annotation and raise_on_failure:
            raise ValueError(
                "LabelInterface.generate_sample_annotation failed to generate a valid annotation"
            )
    ...
    def validate_config_using_summary(self, summary, strict=False):
        """Validate current config using Synapse Project Summary"""
    ...
    def validate_annotations_consistency(self, annotations_summary):
        """ """
        # annotations_summary is coming from Synapse Project Summary, it's
