...
from synapse_sdk.converter.imports import yolo as import_yolo
...
def test_base_import_yolo():
    ...
    with open(out_json_file, "r") as f:
        synapse_data = json.loads(f.read())

    assert len(synapse_data) == len(img_files), "some file imports did not succeed!"
