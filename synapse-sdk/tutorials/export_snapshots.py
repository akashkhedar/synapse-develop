...
- Convert the export to a specified format (optional if SYNAPSE_EXPORT_TYPE is specified)
- Poll for conversion completion
- Download the resulting converted file

Environment variables used:
- SYNAPSE_URL: Your Synapse URL (e.g. https://app.synapse.io)
- SYNAPSE_API_KEY: Personal access token (go to your account and generate it)
- SYNAPSE_PROJECT_ID: Numeric project ID
- SYNAPSE_EXPORT_TYPE: The type of export (CSV, COCO, JSON, etc.) to convert to (optional, default is JSON)

Usage:
```bash
SYNAPSE_URL=https://app.synapse.io SYNAPSE_API_KEY=your_api_key \
SYNAPSE_PROJECT_ID=123456 SYNAPSE_EXPORT_TYPE=CSV \
...
def export_and_download_snapshot():
    ...
    project_id_str = os.getenv("SYNAPSE_PROJECT_ID")

    if not base_url or not api_key or not project_id_str:
        logger.error("set SYNAPSE_URL, SYNAPSE_API_KEY and SYNAPSE_PROJECT_ID env vars")
        sys.exit(1)

    try:
        project_id = int(project_id_str)
    except ValueError:
        logger.error("SYNAPSE_PROJECT_ID must be an integer")
        sys.exit(1)
...
def convert_snapshot(export_type: str, export_id: str = None):
    ...
    The conversion API is used to start a conversion on the Synapse backend, then we poll export status
...
if __name__ == "__main__":
    ...
    export_type = os.getenv("SYNAPSE_EXPORT_TYPE", None)
