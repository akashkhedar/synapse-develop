...
"""
Django Base settings for Synapse.
...
from synapse.core.utils.params import get_bool_env, get_env, get_env_list, has_env
...
LOGGING = {
    ...
    "formatters": {
        "json": {
            "()": "synapse.core.utils.formatter.CustomJsonFormatter",
...
from synapse.core.utils.io import get_data_dir
...
if DOMAIN_FROM_REQUEST:
    ...
    if HOSTNAME and not HOSTNAME.startswith("/"):
        raise ImproperlyConfigured(
            "SYNAPSE_HOST must be a subpath if DOMAIN_FROM_REQUEST is True"
        )
...
DJANGO_DB = "default"
DATABASE_NAME_DEFAULT = os.path.join(BASE_DATA_DIR, "synapse.sqlite3")
...
DATABASES_ALL = {
    ...
    DJANGO_DB_MYSQL: {
        ...
        "NAME": get_env("MYSQL_NAME", "Synapse"),
...
# API Rate Limiting (switch to non-blocking middleware for soft limits)
API_RATE_LIMIT_HARD_BLOCK = get_bool_env("API_RATE_LIMIT_HARD_BLOCK", True)
API_RATE_LIMIT_MULTIPLIER = int(get_env("API_RATE_LIMIT_MULTIPLIER", 10))

# Free tier API limits per day
API_FREE_READ_LIMIT = int(get_env("API_FREE_READ_LIMIT", 10000))
API_FREE_WRITE_LIMIT = int(get_env("API_FREE_WRITE_LIMIT", 1000))
API_FREE_EXPORT_LIMIT = int(get_env("API_FREE_EXPORT_LIMIT", 100))

# Security deposit settings
BILLING_BASE_DEPOSIT = Decimal(get_env("BILLING_BASE_DEPOSIT", "500"))
BILLING_STORAGE_FEE_PER_GB = Decimal(get_env("BILLING_STORAGE_FEE_PER_GB", "10"))
BILLING_ANNOTATION_BUFFER = Decimal(get_env("BILLING_ANNOTATION_BUFFER", "1.5"))

# Storage billing (free tier + tiered pricing)
BILLING_FREE_STORAGE_GB = int(get_env("BILLING_FREE_STORAGE_GB", 5))
BILLING_STORAGE_TIER_1_RATE = Decimal(
    get_env("BILLING_STORAGE_TIER_1_RATE", "5")
)  # 5-50 GB
BILLING_STORAGE_TIER_2_RATE = Decimal(
    get_env("BILLING_STORAGE_TIER_2_RATE", "3")
)  # 50+ GB

# Project lifecycle
PROJECT_DORMANT_DAYS = int(get_env("PROJECT_DORMANT_DAYS", 30))
PROJECT_GRACE_PERIOD_DAYS = int(get_env("PROJECT_GRACE_PERIOD_DAYS", 30))
...
if REDIS_PASSWORD:
    _rq_queue_config["PASSWORD"] = REDIS_PASSWORD

RQ_QUEUES = {
    "critical": _rq_queue_config.copy(),
    "high": _rq_queue_config.copy(),
    "default": _rq_queue_config.copy(),
    "low": _rq_queue_config.copy(),
}
...
SPECTACULAR_SETTINGS = {
    "TITLE": "Synapse API",
    "DESCRIPTION": "Synapse API for data annotation and labeling",
    "VERSION": "",
...
EMAIL_TIMEOUT = int(get_env("EMAIL_TIMEOUT", "30"))
...
# EDITOR = synapse-frontend repository
...
REACT_APP_ROOT = os.path.join(BASE_DIR, "../../web/dist/apps/Synapse")
...
TASK_DATA_PER_BATCH = int(
    get_env("TASK_DATA_PER_BATCH", 50 * 1024 * 1024)
)  # 50 MB in bytes
...
SHOW_TRACEBACK_FOR_EXPORT_CONVERTER = get_bool_env(
    "SHOW_TRACEBACK_FOR_EXPORT_CONVERTER", True
)
...
if (
    ...
    from synapse.io_storages.localfiles.functions import (
...
FEATURE_FLAGS_GET_USER_REPR_FROM_ORGANIZATION = (
    "core.feature_flags.utils.get_user_repr_from_organization"
)
...
STORAGE_EXISTED_COUNT_BATCH_SIZE = int(
    get_env("STORAGE_EXISTED_COUNT_BATCH_SIZE", 1000)
)
...
REAL_HOSTNAME = os.getenv(
    "HOSTNAME"
)  # we have to use getenv, because we don't use SYNAPSE_ prefix
...
if ENABLE_CSP := get_bool_env("ENABLE_CSP", True):
    ...
    MIDDLEWARE.append("core.middleware.SynapseCspMiddleware")
...
RESOLVER_PROXY_MAX_RANGE_SIZE = int(
    get_env("RESOLVER_PROXY_MAX_RANGE_SIZE", 8 * 1024 * 1024)
)
...
RESOLVER_PROXY_ENABLE_ETAG_CACHE = get_bool_env(
    "RESOLVER_PROXY_ENABLE_ETAG_CACHE", True
)
...
USER_ACTIVITY_REDIS_KEY_PREFIX = get_env(
    "USER_ACTIVITY_REDIS_KEY_PREFIX", "user_activity"
)
...
# Base FSM (Finite State Machine) Configuration for Synapse
