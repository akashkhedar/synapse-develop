...
class SynapseAPIException(APIException):
    status_code = status.HTTP_500_INTERNAL_SERVER_ERROR
    default_detail = 'Unknown error'


class SynapseDatabaseException(SynapseAPIException):
    default_detail = 'Error executing database query'


class SynapseDatabaseLockedException(SynapseAPIException):
...
class ProjectExistException(SynapseAPIException):
    status_code = status.HTTP_422_UNPROCESSABLE_ENTITY
    default_detail = 'Project with the same title already exists'


class InvalidUploadUrlError(SynapseAPIException):
    default_detail = (
