...
def find_file(file):
    return find_node('synapse', file, 'file')


def find_dir(directory):
    return find_node('synapse', directory, 'dir')
...
def validate_upload_url(url, block_local_urls=True):
    """Utility function for defending against SSRF attacks. Raises
        ...
        - SynapseApiException if the hostname cannot be resolved
    ...
    try:
        ...
        raise SynapseAPIException(f"Can't resolve hostname {domain}")
