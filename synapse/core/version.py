...
SYNAPSE_VERSION_FILE = "synapse-version_.py"
...
def _read_py(ls=False):
    ...
    try:
        version_module = import_version_module(
            SYNAPSE_VERSION_FILE if ls else VERSION_FILE
        )
...
def get_git_commit_info(skip_os=True, ls=False):
    ...
    try:
        ...
        try:
            desc = run(
                "git describe --long --tags --always --dirty", stderr=STDOUT, shell=True
            ).decode("utf-8")
            info = {
                "message": run("git show -s --format=%s", stderr=STDOUT, shell=True)
                .strip()
                .decode("utf8"),
                "commit": run("git show -s --format=%H", stderr=STDOUT, shell=True)
                .strip()
                .decode("utf8"),
                "date": run(
        ...
        if not skip_os:
            ...
            with open("/etc/os-release") as f:
                os_version = "".join(
                    str(s).split("=", 1)[1].rstrip().strip('"').replace(".", "")
                    for s in f
                    if str(s).startswith(keys)
                )
                version += "." + os_version
