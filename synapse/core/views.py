...
def main(request, **kwargs):
    """Main view that serves the React application for all routes"""
    user = request.user
    ...
    is_public_route = any(
        request.path == route or request.path.startswith(f"{route}/")
        for route in public_routes
    )

    if user.is_authenticated:
        # Skip organization check for annotators (they don't have organizations)
        if not (hasattr(user, "is_annotator") and user.is_annotator):
            if (
    ...
    # Not authenticated - show landing page template for public routes
    if is_public_route:
        return render(request, "landing.html")
...
def api_whoami(request):
    ...
    user = request.user
    is_annotator = user.is_annotator if hasattr(user, "is_annotator") else False
    is_client = user.is_client if hasattr(user, "is_client") else False
    is_expert = user.is_expert if hasattr(user, "is_expert") else False
...
def version_page(request):
    ...
    # from synapse.core.utils.common import check_for_the_latest_version
...
def heidi_tips(request):
    ...
    url = "https://raw.githubusercontent.com/Synapse/synapse/refs/heads/develop/web/apps/Synapse/src/components/HeidiTips/liveContent.json"
    ...
    # This is done this way to catch thrown exceptions from the request itself which will occur for air-gapped environments
        ...
        return HttpResponse(
            json.dumps(content), content_type="application/json", status=status_code
        )
...
def feature_flags(request):
    ...
    flags["$system"] = {
        ...
        "CLOUD_INSTANCE": (
            settings.CLOUD_INSTANCE if hasattr(settings, "CLOUD_INSTANCE") else None
        ),
    }
