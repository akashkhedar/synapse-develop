...
@extend_schema(
    tags=["Export"],
    summary="Get export cost estimate",
    description="Calculate the credit cost for exporting project data before actually performing the export.",
    parameters=[
        OpenApiParameter(
            name="id",
        ...
        OpenApiParameter(
            ...
            location="query",
            description="If true, estimate cost for all tasks. If false, only count finished tasks.",
        ),
    ],
    responses={
        200: OpenApiResponse(
            description="Export cost estimate",
            response={
                "type": "object",
                "properties": {
                    ...
                    "rate_per_task": {"type": "number"},
                },
            },
        )
    },
)
class ExportCostEstimateAPI(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        download_all_tasks = (
            request.GET.get("download_all_tasks", "false").lower() == "true"
        )

        # Count tasks
        query = Task.objects.filter(project=project)
        if not download_all_tasks:
        ...
        # Calculate cost
        rate_per_task = 0.1
        credits_required = task_count * rate_per_task
        ...
        try:
            ...
            current_balance = float(billing.available_credits)
            balance_after = current_balance - credits_required
...
@extend_schema(
    tags=["Export"],
    summary="Get pending annotator payments for export",
    description="""
    Get a summary of pending final payments that will be released to annotators when 
    annotations are downloaded/exported. This represents the final 20% of the 40-40-20 
    payment split:
    - 40% immediate: Released on annotation submission
    - 40% consensus: Released after consensus validation  
    - 20% final: Released when client downloads annotations
    """,
    parameters=[
        OpenApiParameter(
            name="id",
...
class ExportPendingPaymentsAPI(generics.RetrieveAPIView):
    """Get pending final payments that will be released on export download"""
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            pending_summary = PaymentService.get_pending_final_payments(project=project)
            return Response(pending_summary)
        except Exception as e:
...
class ExportAPI(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Check and deduct credits before export
        try:
            from billing.services import CreditService

            organization = request.user.active_organization

            credit_result = CreditService.check_and_deduct_export_credits(
                organization=organization,
                task_count=task_count,
            ...
            logger.info(
                f"Export credits deducted: {credit_result['credits_deducted']} credits "
                f"for {task_count} tasks. Remaining: {credit_result['remaining_credits']}"
            )
        except Exception as e:
            from billing.services import InsufficientCreditsError

            if isinstance(e, InsufficientCreditsError):
                logger.warning(f"Export blocked due to insufficient credits: {str(e)}")
                return Response(
                    {
                        "error": "Insufficient credits",
                        "detail": str(e),
                        "required_credits": task_count * 0.1,
                        "task_count": task_count,
                    },
                    status=status.HTTP_402_PAYMENT_REQUIRED,
                )
            else:
                # If billing module has issues, log but continue export
                logger.warning(f"Credit deduction failed (continuing export): {str(e)}")
        ...
        # Release final payments (20%) for exported annotations
        try:
            from annotators.payment_service import PaymentService

            payment_result = PaymentService.release_final_payments_on_download(
                project=project,
                task_ids=list(task_ids),
                downloaded_by=request.user,
            )
            logger.info(
                f"Final payments released on export: {payment_result['processed_count']} assignments, "
                f"₹{payment_result['total_released']} total for project {project.id}"
            )
        except Exception as e:
            # Log but don't block export if payment release fails
            logger.error(f"Error releasing final payments on export: {str(e)}")
...
#         description="""
#         Retrieve a list of files exported from the Synapse UI using the Export button on the Data Manager page.
...
class ExportListAPI(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        # Estimate task count for credit check
        task_count = project.tasks.count()
        if task_filter_options:
            # Could refine this based on filters, but using total count for simplicity
            pass

        # Check credits before creating export
        try:
            from billing.services import CreditService
            ...
            logger.info(
                f"Export snapshot credits deducted: {credit_result['credits_deducted']} credits "
                f"for {task_count} tasks. Remaining: {credit_result['remaining_credits']}"
            )
        except Exception as e:
            from billing.services import InsufficientCreditsError
            ...
            if isinstance(e, InsufficientCreditsError):
                logger.warning(
                    f"Export snapshot blocked due to insufficient credits: {str(e)}"
                )
                ...
                # If billing module has issues, log but continue export
                logger.warning(
                    f"Credit deduction failed for snapshot (continuing): {str(e)}"
                )
...
class ExportDownloadAPI(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Release final payments (20%) for exported annotations
        # Only release if not already released for this export snapshot
        try:
            from annotators.payment_service import PaymentService

            # Check if payments were already released for this snapshot
            snapshot_metadata = snapshot.meta or {}
            if not snapshot_metadata.get("payments_released"):
                payment_result = PaymentService.release_final_payments_on_download(
                    project=project,
                    task_ids=None,  # Release for all tasks in project with eligible assignments
                    downloaded_by=request.user,
                )

                # Mark payments as released for this snapshot
                if payment_result.get("processed_count", 0) > 0:
                    snapshot.meta = snapshot_metadata
                    snapshot.meta["payments_released"] = True
                    snapshot.meta["payments_released_at"] = timezone.now().isoformat()
                    snapshot.meta["payments_released_by"] = request.user.email
                    snapshot.meta["payment_summary"] = payment_result
                    snapshot.save(update_fields=["meta"])

                logger.info(
                    f"Final payments released on snapshot download: {payment_result['processed_count']} assignments, "
                    f"₹{payment_result['total_released']} total for project {project.id}"
                )
        except Exception as e:
            # Log but don't block download if payment release fails
            logger.error(
                f"Error releasing final payments on snapshot download: {str(e)}"
            )
...
def async_convert(
    ...
    **kwargs,
    ...
    converted_file = snapshot.convert_file(
