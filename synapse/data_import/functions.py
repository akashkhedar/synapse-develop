    ...
    **kwargs,
    ...
    with transaction.atomic():
        try:
            ...
            logger.error(
                f"ProjectImport with id {import_id} not found, import processing failed"
            )
            return
    ...
    tasks, file_upload_ids, found_formats, data_columns = load_tasks_for_async_import(
        project_import, user
    )
    ...
    if project_import.preannotated_from_fields:
        ...
        tasks = reformat_predictions(
            tasks, project_import.preannotated_from_fields, project, raise_errors
        )
    ...
    if project_import.commit_to_project:
        with transaction.atomic():
            ...
            serializer = ImportApiSerializer(
                data=tasks, many=True, context={"project": project}
            )
    ...
    try:
        from annotators.signals import auto_assign_on_tasks_imported

        auto_assign_on_tasks_imported(project.id)
    except Exception as e:
        logger.warning(f"Auto-assignment after async import failed: {e}")
...
def reformat_predictions(
    tasks, preannotated_from_fields, project=None, raise_errors=False
):
    """
    ...
    if project:
        try:
            ...
            logger.warning(
                f"Could not create LabelInterface for project {project.id}: {e}"
            )
    ...
    for task_index, task in enumerate(tasks):
        ...
        for field in preannotated_from_fields:
            if field not in task_data:
                validation_errors.append(
                    f"Task {task_index}: Preannotated field '{field}' not found in task data"
                )
                continue
            ...
            if value is not None:
                ...
                if isinstance(value, dict):
                    ...
                    if prediction_type in ["rating", "number", "datetime"]:
                        ...
                        prediction_value = {
                            value_key: [value] if not isinstance(value, list) else value
                        }
...
def _async_reimport_background_streaming(reimport, project, organization_id, user):
    ...
    try:
        ...
        for (
            ...
            batch_formats,
            batch_columns,
        ) in FileUpload.load_tasks_from_uploaded_files_streaming(
            project,
            reimport.file_upload_ids,
            files_as_tasks_list=reimport.files_as_tasks_list,
            ...
            batch_number += 1
            logger.info(
                f"Processing batch {batch_number} with {len(batch_tasks)} tasks for reimport {reimport.id}"
            )

            # Process batch in transaction
            with transaction.atomic():
                # Lock summary for update to avoid race conditions
                summary = ProjectSummary.objects.select_for_update().get(
                    project=project
                )
        ...
        logger.info(
            f"Streaming reimport {reimport.id} completed: {total_task_count} tasks imported"
        )
        ...
        logger.error(
            f"Error in streaming reimport {reimport.id}: {str(e)}", exc_info=True
        )
...
def _async_import_background_streaming(project_import, user):
    try:
        ...
        batch_number = 0
        streaming_generator = load_tasks_for_async_import_streaming(
            project_import, user, batch_size
        )
        ...
        for (
            ...
            if project_import.preannotated_from_fields:
                ...
                logger.info(
                    f"Reformatting predictions with raise_errors: {raise_errors}"
                )
                batch_tasks = reformat_predictions(
                    batch_tasks,
                    project_import.preannotated_from_fields,
                    project,
                    raise_errors,
                )
                    ...
                    if "predictions" in task:
                        for j, prediction in enumerate(task["predictions"]):
                            try:
                                ...
                                validation_errors.append(error_msg)
                                logger.error(
                                    f"Exception during validation: {error_msg}"
                                )
            ...
            if project_import.commit_to_project:
                with transaction.atomic():
                    ...
                    serializer = ImportApiSerializer(
                        data=batch_tasks, many=True, context={"project": project}
                    )
                    serializer.is_valid(raise_exception=True)
                    batch_db_tasks = serializer.save(project_id=project.id)
        ...
        logger.info(
            f"Streaming import {project_import.id} completed: {total_task_count} tasks imported"
        )

        # Trigger auto-assignment after streaming import completes
        try:
            from annotators.signals import auto_assign_on_tasks_imported

            auto_assign_on_tasks_imported(project.id)
        except Exception as e:
            logger.warning(f"Auto-assignment after streaming import failed: {e}")

    except Exception as e:
        logger.error(
            f"Error in streaming import {project_import.id}: {str(e)}", exc_info=True
        )
        project_import.status = ProjectImport.Status.FAILED
...
def async_reimport_background(reimport_id, organization_id, user, **kwargs):
    ...
    with transaction.atomic():
        try:
            ...
            logger.error(
                f"ProjectReimport with id {reimport_id} not found, import processing failed"
            )
            return
    ...
    if flag_set(
        "fflag_fix_back_plt_838_reimport_memory_improvement_05082025_short", user="auto"
    ):
        ...
        with transaction.atomic():
            ...
            project.remove_tasks_by_file_uploads(reimport.file_upload_ids)
            serializer = ImportApiSerializer(
                data=tasks, many=True, context={"project": project, "user": user}
            )
            serializer.is_valid(raise_exception=True)
            tasks = serializer.save(project_id=project.id)
            emit_webhooks_for_instance(
                organization_id, project, WebhookAction.TASKS_CREATED, tasks
            )
        ...
        # Trigger auto-assignment after async reimport completes
        try:
            from annotators.signals import auto_assign_on_tasks_imported

            auto_assign_on_tasks_imported(project.id)
        except Exception as e:
            logger.warning(f"Auto-assignment after async reimport failed: {e}")
