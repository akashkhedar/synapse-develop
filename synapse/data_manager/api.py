...
from core.rbac import resolve_actor
...
@method_decorator(
    ...
    decorator=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="id", type=OpenApiTypes.STR, location="path", description="View ID"
            ),
        ],
        extensions={
            "x-fern-sdk-group-name": "views",
...
class ViewAPI(viewsets.ModelViewSet):
    ...
    @action(
        detail=False,
        methods=["delete"],
        permission_required=all_permissions.views_reset,
    )
    def reset(self, request):
        serializer = ViewResetSerializer(
            data=(
                ...
                else {"project": request.query_params.get("project")}
            )
        )
        serializer.is_valid(raise_exception=True)
        project = generics.get_object_or_404(
            Project.objects.for_user(request.user),
            pk=serializer.validated_data["project"].id,
        )
...
class TaskPagination(PageNumberPagination):
    ...
    async def async_paginate_queryset(self, queryset, request, view=None):
        ...
        self.total_predictions = await sync_to_async(
            predictions_count_qs.count, thread_sensitive=True
        )()

        annotations_count_qs = Annotation.objects.filter(
            task_id__in=queryset, was_cancelled=False
        )
        self.total_annotations = await sync_to_async(
            annotations_count_qs.count, thread_sensitive=True
        )()
        return await sync_to_async(super().paginate_queryset, thread_sensitive=True)(
...
def ensure_default_view(project, user):
...
class TaskListAPI(generics.ListCreateAPIView):
    ...
    def get_task_queryset(self, request, prepare_params):
        # Let Synapse build the prepared queryset first
        queryset = Task.prepared.only_filtered(prepare_params)

        # Check if user is an annotator or expert and filter accordingly
        user = request.user

        # Check if user has annotator profile
        annotator_profile = getattr(user, "annotator_profile", None)
        expert_profile = getattr(user, "expert_profile", None)

        # For annotators (who are not also admins/clients), only show assigned tasks
        if (
            annotator_profile
            and not user.is_staff
            and not getattr(user, "is_client", False)
        ):
            # If user is also an expert, they might be viewing as expert
            # Check if there's an expert_view param
            is_expert_view = request.GET.get("expert_view", "").lower() == "true"

            if expert_profile and is_expert_view:
                # Expert viewing for review - only show tasks with ExpertReviewTask
                from annotators.models import ExpertReviewTask

                assigned_task_ids = ExpertReviewTask.objects.filter(
                    expert=expert_profile, status__in=["pending", "in_review"]
                ).values_list("task_id", flat=True)
                queryset = queryset.filter(id__in=assigned_task_ids)
            else:
                # Regular annotator view - only show assigned tasks
                assigned_task_ids = TaskAssignment.objects.filter(
                    annotator=annotator_profile, status__in=["assigned", "in_progress"]
                ).values_list("task_id", flat=True)
                queryset = queryset.filter(id__in=assigned_task_ids)

        # For experts (who are not annotators), only show tasks with ExpertReviewTask
        elif expert_profile and not annotator_profile and not user.is_staff:
            from annotators.models import ExpertReviewTask
    ...
    def get(self, request):
        raw_view_get = request.GET.get("view")
        raw_view_data = (
            request.data.get("view") if isinstance(request.data, dict) else None
        )
        project_pk = int_from_request(request.GET, "project", 0) or int_from_request(
            request.data, "project", 0
        )
        if raw_view_get == "annotator-default" or raw_view_data == "annotator-default":
            project = generics.get_object_or_404(Project, pk=project_pk)
            view = ensure_default_view(project, request.user)
            view_pk = view.id
        else:
        ...
        if project_pk:
            ...
            return Response(
                {"detail": "Neither project nor view id specified"}, status=404
            )
...
def get_actor_context(request):
    return resolve_actor(request)


class DataManagerUsersAPI(APIView):
    permission_required = all_permissions.projects_view
    ...
    def get(self, request):
        ...
        users = project.organization.members.all()
...
class ProjectStateAPI(APIView):
    ...
    def get(self, request):
        ...
        # Check if user is annotator or client
        if request.user.is_client or not request.user.is_annotator:
            # Clients and admins - existing behavior
            self.check_object_permissions(request, project)
        elif request.user.is_annotator:
            # Annotators - check if they're a member of the project
            is_member = project.has_collaborator_enabled(request.user)

            if not is_member:
                return Response(
                    {"detail": "Annotator is not assigned to this project"}, status=403
                )
        else:
            return Response({"detail": "Not authenticated"}, status=401)

        # ✅ IMPORTANT: always return REAL project data
        data = ProjectSerializer(
            project,
            context={"request": request},
        ).data

        role = "annotator" if request.user.is_annotator else "client"
        data.update(
            {
                # annotators are restricted by backend anyway
                "can_delete_tasks": role == "client",
                "can_manage_annotations": role == "client",
                "can_manage_tasks": role == "client",
...
class ProjectActionsAPI(APIView):
    ...
    def get(self, request):
        ...
        project = generics.get_object_or_404(Project, pk=pk)
        self.check_object_permissions(request, project)
        role, user, annotator_profile, expert_profile = resolve_actor(request)
        return Response(get_all_actions(user, project))
    ...
    def post(self, request):
        ...
        if not action_id:
            return Response(
                {"detail": "No action id, use ?id=<action-id>"},
                status=422,
            )
        ...
        ) and action_id not in ["next_task", "remove_duplicates"]:
            prepare_params.ordering = []

        queryset = Task.prepared.only_filtered(
        ...
        if role == "annotator":
            queryset = queryset.filter(
                annotator_assignments__annotator=annotator_profile
            ).distinct()

        if not prepare_params.ordering:
