...
def get_all_columns(project, *_):
    ...
    logger.info(
        f"get_all_columns: project_id={project.id} {all_data_columns=} {data_types=}"
    )
    if all_data_columns:
        data_types.update(
            {key: "Unknown" for key in all_data_columns if key not in data_types}
        )
    ...
    logger.info(
        f"get_all_columns: project_id={project.id} {data_types=} {project_data_types=}"
    )

    for key, data_type in list(
        data_types.items()
        ...
        column = {
            ...
            "visibility_defaults": {
                "explore": True,
                "labeling": key in project_data_types
                or key == settings.DATA_UNDEFINED_NAME,
            },
...
def get_prepare_params(request, project):
    ...
    * selectedItems, filters, ordering if they are in request and there is no view id
    ...
    # Support synthetic annotator-default view id (string) returned to annotators
    raw_view_get = request.GET.get("view")
    raw_view_data = request.data.get("view") if isinstance(request.data, dict) else None
    if raw_view_get == "annotator-default" or raw_view_data == "annotator-default":
        view_id = 0
    else:
        view_id = int_from_request(request.GET, "view", 0) or int_from_request(
            request.data, "view", 0
        )
    if view_id > 0:
