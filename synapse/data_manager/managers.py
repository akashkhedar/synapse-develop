...
from synapse.core.utils.params import cast_bool_from_str
...
def get_fields_for_filter_ordering(prepare_params):
    ...
    if prepare_params.ordering:
        ordering_field_name = (
            prepare_params.ordering[0].replace("tasks:", "").replace("-", "")
        )
...
def get_fields_for_evaluation(prepare_params, user, skip_regular=True):
    ...
    from synapse.data_manager.functions import TASKS
    ...
    all_columns = set(
        [
            TASKS + ("data." if c.get("parent", None) == "data" else "") + c["id"]
            for c in all_columns["columns"]
        ]
    )

    if fields:
        # If hiddenColumns provided, compute shown = all_columns - hidden intersection
        ...
        # No hiddenColumns specified — include all data-manager columns so clients
        # and annotators see the same annotated fields by default.
        shown = {c[len(TASKS) :] for c in all_columns} - {"data"}
        result = set(result) | shown
        ...
        if field_name.startswith("data__"):
            ...
            if numeric_ordering is True:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to apply numeric ordering for field {json_field}: {e}"
                    )
            ...
            f = (
                F("ordering_field").asc(nulls_last=True)
                if ascending
                else F("ordering_field").desc(nulls_last=True)
            )

        elif field_name == "state":
            state_choices = get_state_choices("task")
            whens = [
                When(current_state=state, then=Value(i + 1))
                for i, state in enumerate(state_choices.values)
            ]
            queryset = queryset.annotate(
                state_order=Case(
            ...
            f = (
                F("state_order").asc(nulls_last=True)
                if ascending
                else F("state_order").desc(nulls_last=True)
            )
        else:
            f = (
                F(field_name).asc(nulls_last=True)
                if ascending
                else F(field_name).desc(nulls_last=True)
            )
    ...
    if field_name == "annotations_results":
        subquery = Q(
            id__in=Annotation.objects.annotate(
                json_str=RawSQL("cast(result as text)", "")
            )
...
def apply_filters(queryset, filters, project, request):
    ...
    for parent_filter in filters.items:
        filter_line = (
            [parent_filter, parent_filter.child_filter]
        ...
        for _filter in filter_line:
            is_child_filter = (
                parent_filter.child_filter is not None
                and _filter is parent_filter.child_filter
            )
            ...
            # updated_by
            result = add_user_filter(
                field_name == "updated_by", "updated_by", _filter, filter_expressions
            )
            if result == "continue":
                continue
            ...
            if field_name in ["annotations_results", "predictions_results"]:
                result = add_result_filter(
                    field_name, _filter, filter_expressions, project
                )
                if result == "exit":
            ...
            if field_name == "annotations_ids":
                ...
                if "contains" in _filter.operator:
                    ...
                    _filter.operator = (
                        "in_list" if _filter.operator == "contains" else "not_in_list"
                    )
            ...
            if (
                field_name == "predictions_model_versions"
                and _filter.operator == Operator.CONTAINS
            ):
                q = Q()
                ...
                field_name == "predictions_model_versions"
                and _filter.operator == Operator.NOT_CONTAINS
            ):
                q = Q()
                for value in _filter.value:
                    q &= ~Q(predictions__model_version__contains=value)
                ...
                field_name == "predictions_model_versions"
                and _filter.operator == Operator.EMPTY
            ):
                value = cast_bool_from_str(_filter.value)
            ...
            if (
                clean_field_name
                in ("total_predictions", "total_annotations", "cancelled_annotations")
                and _filter.operator == "empty"
            ):
                _filter.operator = (
                    "equal" if cast_bool_from_str(_filter.value) else "not_equal"
                )
                _filter.value = 0
            ...
            if queryset.exists():
                value_type = type(
                    queryset.values_list(field_name, flat=True)[0]
                ).__name__

            if (
                value_type == "list" or value_type == "tuple"
            ) and "equal" in _filter.operator:
            ...
            # regex pattern check
                ...
                try:
                    ...
                    logger.info(
                        "Incorrect regex for filter: %s: %s", _filter.value, str(e)
                    )
                    return queryset.none()
    ...
    resolved_filter_lines = [
        reduce(lambda x, y: x & y, fle) for fle in filter_line_expressions
    ]
...
class TaskQuerySet(FSMStateQuerySetMixin, models.QuerySet):
    ...
    def prepared(self, prepare_params=None):
        ...
        if (
            prepare_params.selectedItems.all is False
            and prepare_params.selectedItems.included
        ):
            queryset = queryset.filter(id__in=prepare_params.selectedItems.included)

        # excluded selected items
        elif (
            prepare_params.selectedItems.all is True
            and prepare_params.selectedItems.excluded
        ):
...
def newest_annotation_subquery() -> Subquery:
    ...
    newest_annotations = Annotation.objects.filter(task=OuterRef("pk")).order_by("-id")[
        :1
    ]
...
def base_annotate_completed_at(queryset: TaskQuerySet) -> TaskQuerySet:
    return queryset.annotate(
        completed_at=Case(When(is_labeled=True, then=newest_annotation_subquery()))
    )
...
def annotated_completed_at_considering_agreement_threshold(queryset):
    ...
    project_exists = (
        is_lse_project and hasattr(queryset, "project") and queryset.project is not None
    )
    ...
    max_additional_annotators_assignable = lse_project[
        ...
        completed_at_case = Case(
            When(
                ...
                & (
                    ...
                    | Q(
                        annotator_count__gte=(
                            F("overlap") + max_additional_annotators_assignable
                        )
                    )
                ),
...
def annotate_storage_filename(queryset: TaskQuerySet) -> TaskQuerySet:
    from synapse.data_manager.functions import intersperse

    storage_key_names = [F(s + "__key") for s in settings.IO_STORAGES_IMPORT_LINK_NAMES]
    return queryset.annotate(
        storage_filename=Concat(
            *intersperse(storage_key_names, Value(";")), output_field=TextField()
        )
    )
    ...
    if settings.DJANGO_DB == settings.DJANGO_DB_SQLITE:
        return queryset.annotate(
            annotations_results=Coalesce(
                GroupConcat("annotations__result"),
                Value(""),
        ...
        return queryset.annotate(
            annotations_results=ArrayAgg(
                "annotations__result", distinct=True, default=Value([])
            )
        )
    ...
    if settings.DJANGO_DB == settings.DJANGO_DB_SQLITE:
        return queryset.annotate(
            predictions_results=Coalesce(
                GroupConcat("predictions__result"),
                Value(""),
                output_field=models.CharField(),
            )
        )
        ...
        return queryset.annotate(
            predictions_results=ArrayAgg(
                "predictions__result", distinct=True, default=Value([])
            )
        )
...
def annotate_annotators(queryset):
    if settings.DJANGO_DB == settings.DJANGO_DB_SQLITE:
        return queryset.annotate(
            annotators=Coalesce(
                GroupConcat("annotations__completed_by"),
                Value(""),
                output_field=models.CharField(),
            )
        )
        ...
        return queryset.annotate(
            annotators=ArrayAgg(
                "annotations__completed_by", distinct=True, default=Value([])
            )
        )
...
def annotate_predictions_score(queryset):
    ...
    if flag_set(
        ...
        first_task.project.organization.created_by,
    ):
        model_versions = list(
        ...
        if model_version is None:
            ...
            return queryset.annotate(
                predictions_score=Avg(
                    "predictions__score",
                    filter=Q(predictions__model_version=model_version),
                )
            )
...
def annotate_annotations_ids(queryset):
    if settings.DJANGO_DB == settings.DJANGO_DB_SQLITE:
        return queryset.annotate(
            annotations_ids=GroupConcat(
                "annotations__id", output_field=models.CharField()
            )
        )
    else:
        return queryset.annotate(
            annotations_ids=ArrayAgg("annotations__id", default=Value([]))
        )
...
def annotate_predictions_model_versions(queryset):
    if settings.DJANGO_DB == settings.DJANGO_DB_SQLITE:
        return queryset.annotate(
            predictions_model_versions=GroupConcat(
                "predictions__model_version", output_field=models.CharField()
            )
        )
    else:
        return queryset.annotate(
            predictions_model_versions=ArrayAgg(
                "predictions__model_version", default=Value([])
            )
        )
...
def annotate_draft_exists(queryset):
    ...
    return queryset.annotate(
        draft_exists=Exists(AnnotationDraft.objects.filter(task=OuterRef("pk")))
    )
...
class PreparedTaskManager(models.Manager):
    ...
    def annotate_queryset(
        ...
        for field in annotations_map.keys():
            ...
            if (
                field in fields_for_evaluation or all_fields
            ) and field not in excluded_fields_for_evaluation:
    ...
    def only_filtered(self, prepare_params=None):
        ...
        if prepare_params.is_multi_project:
            queryset = TaskQuerySet(self.model).filter(
                project__in=prepare_params.projects
            )
        else:
        ...
        queryset = self.annotate_queryset(
            queryset, fields_for_evaluation=fields_for_filter_ordering, request=request
        )
