...
from synapse.core.utils.common import round_floats
    ...
    def validate_column(self, column: str) -> str:
        ...
        if not column_copy.startswith(required_prefix):
            raise serializers.ValidationError(
                f'Filter "{column}" should start with "{required_prefix}"'
            )
...
class FilterGroupSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        roots = (
            instance.filters.filter(parent__isnull=True)
        ...
        return {
            ...
            "items": [_build_filter_tree(f) for f in roots],
        }
    ...
    def create(self, validated_data):
        with transaction.atomic():
            ...
            if filter_group_data:
                ...
                validated_data["order"] = View.objects.filter(
...
class AnnotationsDMFieldSerializer(AnnotationSerializer):
    completed_by = CompletedByDMSerializerWithGenericSchema(
...
class DataManagerTaskSerializer(TaskSerializer):
    ...
    annotations = AnnotationsDMFieldSerializer(
    ...
    def to_representation(self, obj):
        ...
        # =====================================================================
        # FILTER ANNOTATIONS AND ANNOTATORS BASED ON USER ROLE
        # =====================================================================
        # Annotators can only see their own annotations
        # Experts see consolidated annotation if available
        # Clients can only see after review is complete
        try:
            from annotators.annotation_workflow import AnnotationWorkflowService
            from annotators.models import TaskConsensus, AnnotatorProfile

            # Get user from request context (more reliable than CurrentContext)
            ...
            if user and user.is_authenticated:
                ...
                # Check if expert should see consolidated annotation
                if is_expert and "annotations" in ret:
                    try:
                        consensus = obj.consensus
                        if consensus.consolidated_result and consensus.status in [
                            "review_required",
                            "consensus_reached",
                            "finalized",  # Also show consolidated for finalized tasks
                        ]:
                            # Replace annotations with consolidated annotation
                            consolidated_ann = {
                                "id": "consolidated",
                                ...
                                "ground_truth": False,
                                "created_at": (
                                    consensus.updated_at.isoformat()
                                ...
                                "_consolidation_method": consensus.consolidation_method,
                                "_average_agreement": float(
                ...
                # Filter annotations for non-expert users
                if not is_expert and "annotations" in ret and ret["annotations"]:
                    visible_annotations = (
                        AnnotationWorkflowService.get_visible_annotations(
                            user, obj, include_own=True
                        )
                    )
                ...
                # Filter annotators field - hide for annotators and clients before finalization
                if "annotators" in ret and ret["annotators"]:
                    should_hide_annotators = False

                    if is_annotator and not is_expert and not is_admin:
                        # Annotators cannot see other annotators
                        should_hide_annotators = True
                    elif not is_expert and not is_admin and not task_finalized:
                        # Clients - only see after finalization
                        should_hide_annotators = True

                    if should_hide_annotators:
                        ret["annotators"] = []
            ...
            logging.getLogger(__name__).warning(
                f"Error filtering annotations/annotators: {e}"
            )
            traceback.print_exc()
    ...
    def get_annotators(obj):
        ...
        # Convert user IDs to user objects with necessary fields for frontend
        if annotators:
            from users.models import User

            users = User.objects.filter(id__in=annotators).values(
                "id", "first_name", "last_name", "email", "username", "avatar"
            )
            # Map users by ID and construct annotator objects
            # Note: User properties must be at top level for MobX State Tree's Assignee.preProcessSnapshot
            user_map = {u["id"]: u for u in users}
            result = []
            for user_id in annotators:
                if user_id in user_map:
                    user_data = user_map[user_id]
                    # Include 'id' in user properties so UserOrReference dispatcher recognizes it as a User object
                    # The Assignee.preProcessSnapshot extracts user_id, annotated, review, reviewed
                    # and puts remaining properties (including 'id') into the user object
                    result.append(
                        {
                            "user_id": user_id,
                            "id": user_data[
                                "id"
                            ],  # Must include for UserOrReference to work
...
class ViewOrderSerializer(serializers.Serializer):
    ...
    ids = serializers.ListField(
        ...
        allow_empty=False,
        help_text="A list of view IDs in the desired order.",
    )
