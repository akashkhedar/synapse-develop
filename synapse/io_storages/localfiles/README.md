...
Local Files storage lets self-hosted Synapse read media (images, audio, video, documents) directly from the server's filesystem and write annotation results back to disk. It's designed for air-gapped environments or workflows where data cannot leave the host machine.
...
Variables can be prefixed with `SYNAPSE_` or `HEARTEX_` (checked in that order).

### Community Edition Auto-Detection

When both `LOCAL_FILES_DOCUMENT_ROOT` and `LOCAL_FILES_SERVING_ENABLED` are unset, Community Edition automatically searches for `mydata` or `synapse-data` directories in the current working directory. If found, it enables local file serving with that directory as the document root.

**Docker shortcut:** Mount your host folder to `/synapse/mydata` inside the container to enable local files without setting any environment variables.

### Production Setup

```bash
export SYNAPSE_LOCAL_FILES_SERVING_ENABLED=true
export SYNAPSE_LOCAL_FILES_DOCUMENT_ROOT=/data/synapse

# Directory structure:
# /data/synapse/           ← DOCUMENT_ROOT
# /data/synapse/project1/  ← Storage path for project 1
# /data/synapse/project2/  ← Storage path for project 2
```
...
1. Check environment:
   ```bash
   echo $SYNAPSE_LOCAL_FILES_SERVING_ENABLED
   echo $SYNAPSE_LOCAL_FILES_DOCUMENT_ROOT
   ```
