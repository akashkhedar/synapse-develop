...
from synapse.io_storages.s3.utils import AWS
...
class S3StorageMixin(models.Model):
    ...
    def validate_connection(self, client=None):
        ...
        if self.prefix:
            ...
            if (
                keycount := result.get("KeyCount")
            ) is None or keycount < expected_keycount:
                raise KeyError(
    ...
    def get_bytes_stream(self, uri, range_header=None):
        ...
        try:
            ...
            logger.error(
                f"Error getting direct stream from S3 for uri {uri}: {e}", exc_info=True
            )
            return None, None, {}
