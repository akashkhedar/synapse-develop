...
class OrganizationMember(OrganizationMemberMixin, models.Model):
    ...
    @cached_property
    def is_owner(self):
        return self.role == self.ROLE_OWNER or self.user.id == self.organization.created_by.id
    ...
    def can_manage_members(self):
        """Check if this member can manage other members (remove, promote, demote)"""
...
class Organization(OrganizationMixin, models.Model):
    ...
    def add_user(self, user, role=None):
        ...
        existing_member = OrganizationMember.objects.filter(user=user, organization=self).first()
        
        if existing_member:
            if existing_member.deleted_at is not None:
                # Restore soft-deleted membership - keep original role
                existing_member.deleted_at = None
                existing_member.save(update_fields=['deleted_at'])
                logger.info(f'Restored soft-deleted membership for user {user.email} in organization {self.title}')
        ...
        # Determine role for new membership
        if role is None:
            # If user is the creator, make them owner
