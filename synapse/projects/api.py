...
from core.utils.exceptions import SynapseDatabaseException, ProjectExistException
...
@method_decorator(
    ...
    To perform most tasks with the Synapse API, you must specify the project ID, sometimes referred to as the `pk`.
    To retrieve a list of your Synapse projects, update the following command to match your own environment.
...
@method_decorator(
    ...
    decorator=extend_schema(
        ...
        description="""
    Create a project and set up the labeling interface in Synapse using the API.
...
class ProjectListAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if flag_set(
            "fflag_feat_fit_568_finite_state_management", user=self.request.user
    ...
    def perform_create(self, ser):
        try:
            ...
            from organizations.email_notifications import send_project_created_email

            send_project_created_email(project, project.organization, self.request.user)
            ...
            raise SynapseDatabaseException(
...
class ProjectCountsListAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # Allow annotators to access projects they're members of, otherwise restrict to user's organization
        if self.request.user.is_annotator:
            projects = (
                Project.objects.with_counts(fields=fields)
                .filter(members__user=self.request.user, members__enabled=True)
                .distinct()
            )
        else:
    ...
    def delete(self, request, *args, **kwargs):
        ...
        try:
            ...
            if hasattr(instance, "billing"):
                ...
                # Only process refund if deposit was paid and not already refunded
                if (
            ...
            logger.error(f"Error processing refund during project deletion: {e}")
            refund_result = {"success": False, "error": str(e)}
        ...
        if refund_result and refund_result.get("success"):
            response_data["refund"] = {
                "success": True,
                "amount": refund_result.get("refunded_amount", 0),
                "original_deposit": refund_result.get("original_deposit", 0),
                "consumed": refund_result.get("consumed", 0),
            }
        elif refund_result:
            response_data["refund"] = {
                "success": False,
...
class ProjectNextTaskAPI(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        actor = annotator_profile if role == "annotator" else user
        next_task, queue_info = get_next_task(actor, prepared_tasks, project, dm_queue)
...
class LabelStreamHistoryAPI(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        role, user, annotator_profile, expert_profile = resolve_actor(request)
        history = get_label_stream_history(user, project)
...
class ProjectTaskListAPI(
    ...
    def filter_queryset(self, queryset):
        role, user, annotator_profile, expert_profile = resolve_actor(self.request)
        if role == "annotator" and annotator_profile is None:
            raise Http404
        if role == "expert" and expert_profile is None:
            raise Http404
        ...
        if role == "annotator":
            ...
            # Experts only see tasks assigned to them for review
            from annotators.models import ExpertReviewTask, ExpertProjectAssignment

            project = generics.get_object_or_404(
                Project,
                pk=self.kwargs.get("pk", 0),
            )
            # Check if expert has access to this project
            if not ExpertProjectAssignment.objects.filter(
                expert=expert_profile, project=project
            ).exists():
                raise Http404

            # Only show tasks with pending review assignments
    ...
    def delete(self, request, *args, **kwargs):
        role, _, _, _ = resolve_actor(request)
        if role == "annotator" or role == "expert":
        ...
        logger.info(
            f"calling reset project_id={project.id} ProjectTaskListAPI.delete()"
        )
...
def read_templates_and_groups():
    ...
    if settings.VERSION_EDITION != "Community":
        groups = [
            group for group in groups if group.lower() != "community contributions"
        ]
...
class ProjectSampleTask(generics.RetrieveAPIView):
    ...
    def post(self, request, *args, **kwargs):
        label_config = self.request.data.get("label_config")
        include_annotation_and_prediction = self.request.data.get(
