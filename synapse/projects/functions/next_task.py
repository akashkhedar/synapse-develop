...
should_attempt_ground_truth_first = (
    load_func(settings.SHOULD_ATTEMPT_GROUND_TRUTH_FIRST)
    or _oss_should_attempt_gt_first
)
...
def _get_random_unlocked(
    task_query: QuerySet[Task], user: User, upper_limit=None
) -> Union[Task, None]:
    for task in task_query.order_by("?").only("id")[
        : settings.RANDOM_NEXT_TASK_SAMPLE_SIZE
    ]:
        try:
...
def _try_ground_truth(
    tasks: QuerySet[Task], project: Project, user: User
) -> Union[Task, None]:
    ...
    not_solved_tasks_with_ground_truths = tasks.annotate(
        has_ground_truths=Exists(ground_truth)
    ).filter(has_ground_truths=True)
...
def _try_tasks_with_overlap(
    tasks: QuerySet[Task],
) -> Tuple[Union[Task, None], QuerySet[Task]]:
...
def _try_breadth_first(
    tasks: QuerySet[Task], user: User, project: Project
) -> Union[Task, None]:
    ...
    tasks = tasks.annotate(
        annotations_count=Count("annotations", filter=annotation_filter)
    )
    ...
    not_solved_tasks_labeling_with_max_annotations = (
        not_solved_tasks_labeling_started.filter(reach_max_annotations_count=True)
    )
    if not_solved_tasks_labeling_with_max_annotations.exists():
        # try to complete tasks that are already in progress
        return _get_random_unlocked(
            not_solved_tasks_labeling_with_max_annotations, user
        )
    ...
    if task_with_current_predictions.exists():
        ...
        if cluster_num_solved_map:
            task_with_current_predictions = task_with_current_predictions.annotate(
                cluster_num_solved=Case(
                    *cluster_num_solved_map, default=0, output_field=DecimalField()
                )
            )
        ...
        if num_annotators > 1 and num_tasks_with_current_predictions > 0:
            ...
            next_task = _get_random_unlocked(
                possible_next_tasks,
                user,
                upper_limit=min(num_annotators + 1, num_tasks_with_current_predictions),
            )
        else:
...
def get_not_solved_tasks_qs(
    ...
    user_solved_tasks_array = user.annotations.filter(
        project=project, task__isnull=False
    )
    user_solved_tasks_array = user_solved_tasks_array.distinct().values_list(
        "task__pk", flat=True
    )
    ...
    if postponed_drafts.exists():
        user_postponed_tasks = postponed_drafts.distinct().values_list(
            "task__pk", flat=True
        )
    ...
    if not assigned_flag:
        ...
        if (
            ...
            qs = qs.annotate(
                annotators=Count("annotations__completed_by", distinct=True)
            )

            low_agreement_pred = Q(
                _agreement__lt=lse_project.agreement_threshold, is_labeled=True
            ) | Q(is_labeled=False)
            capacity_pred = Q(
                annotators__lt=F("overlap")
                + (lse_project.max_additional_annotators_assignable or 0)
            )

            if project.show_ground_truth_first:
                gt_subq = Annotation.objects.filter(
                ...
                not_solved_tasks = qs.filter(
                    Q(has_ground_truths=True) | (low_agreement_pred & capacity_pred)
                )
            else:
                not_solved_tasks = qs.filter(low_agreement_pred & capacity_pred)

            prioritized_on_agreement, not_solved_tasks = (
                _prioritize_low_agreement_tasks(not_solved_tasks, lse_project)
            )
...
def _prioritize_low_agreement_tasks(tasks, lse_project):
    ...
    prioritized_low_agreement = tasks.filter(
        _agreement__lt=lse_project.agreement_threshold, is_labeled=True
    )
...
def skipped_queue(next_task, prepared_tasks, project, user, assigned_flag, queue_info):
    if not next_task and project.skip_queue == project.SkipQueue.REQUEUE_FOR_ME:
        ...
        skipped_tasks = (
            user.annotations.filter(q)
            .order_by("updated_at")
            .values_list("task__pk", flat=True)
        )
        if skipped_tasks.exists():
            preserved_order = Case(
                *[When(pk=pk, then=pos) for pos, pk in enumerate(skipped_tasks)]
            )
            skipped_tasks = prepared_tasks.filter(pk__in=skipped_tasks).order_by(
                preserved_order
            )
...
def postponed_queue(
    ...
    if not next_task:
        ...
        postponed_tasks = (
            user.drafts.filter(q)
            .order_by("updated_at")
            .values_list("task__pk", flat=True)
        )
        if postponed_tasks.exists():
            preserved_order = Case(
                *[When(pk=pk, then=pos) for pos, pk in enumerate(postponed_tasks)]
            )
            postponed_tasks = prepared_tasks.filter(pk__in=postponed_tasks).order_by(
                preserved_order
            )
...
def get_next_task(
    ...
    logger.debug(
        f"get_next_task called. user: {user}, project: {project}, dm_queue: {dm_queue}"
    )
    ...
    with conditional_atomic(predicate=db_is_not_sqlite):
        ...
        (
            ...
            user_solved_tasks_array,
            queue_info,
            prioritized_low_agreement,
        ) = get_not_solved_tasks_qs(
            user, project, prepared_tasks, assigned_flag, queue_info, allow_gt_first
        )

        # Honeypot injection: silently inject test tasks for quality control
        # Only for annotators, not during onboarding (GT-first mode)
        if not allow_gt_first and hasattr(user, "annotator_profile"):
            try:
                from annotators.honeypot_service import HoneypotService

                annotator_profile = user.annotator_profile
                if HoneypotService.should_inject_honeypot(annotator_profile, project):
                    honeypot_task = HoneypotService.get_honeypot_task(
                        annotator_profile, project
                    )
                    if honeypot_task:
                        logger.debug(
                            f"User={user} assigned honeypot task {honeypot_task.id}"
                        )
                        honeypot_task.set_lock(user)
                        add_stream_history(honeypot_task, user, project)
                        return honeypot_task, "Honeypot queue"
            except Exception as e:
                logger.warning(f"Honeypot injection failed for user={user}: {e}")
        ...
        if next_task and flag_set(
            "fflag_fix_back_dev_4185_next_task_additional_logging_long", user
        ):
            try:
                ...
                locks = (
                    next_task.locks.count()
                    > project.maximum_annotations - next_task.annotations.count()
                )
                if (
                    next_task.is_labeled
                    or task_overlap_reached
                    or global_overlap_reached
                    or locks
                ):
