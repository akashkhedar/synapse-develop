...
from synapse_sdk._extensions.synapse_tools.core.label_config import (
    parse_config,
)
...
class ProjectManager(models.Manager):
    ...
    def with_counts_annotate(queryset, fields=None, exclude=None):
        ...
        if exclude:
            to_annotate = {
                field: func
                for field, func in to_annotate.items()
...
class Project(ProjectMixin, FsmHistoryStateModel):
    ...
    reveal_preannotations_interactively = models.BooleanField(
        _("reveal_preannotations_interactively"),
        default=False,
    ...
    token = models.CharField(
        _("token"), max_length=256, default=create_hash, null=True, blank=True
    )
    ...
    color = models.CharField(
        _("color"), max_length=16, default="#FFFFFF", null=True, blank=True
    )
    ...
    pinned_at = models.DateTimeField(
        _("pinned at"), null=True, default=None, help_text="Pinned date and time"
    )
    ...
    min_trust_level = models.CharField(
        _("minimum trust level"),
        max_length=20,
        choices=[
            ("new", "New"),
            ("junior", "Junior"),
    ...
    quality_requirement = models.FloatField(
    ...
    max_annotators = models.IntegerField(
        _("max annotators"),
        null=True,
        blank=True,
        help_text="Maximum number of annotators for this project",
    )
    assignment_priority = models.IntegerField(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__original_label_config = (
            self.label_config if "label_config" not in deferred_fields else None
        )
        self.__maximum_annotations = (
            self.maximum_annotations
            if "maximum_annotations" not in deferred_fields
            else None
        )
        self.__overlap_cohort_percentage = (
            self.overlap_cohort_percentage
            if "overlap_cohort_percentage" not in deferred_fields
            else None
        )
        self.__skip_queue = (
            self.skip_queue if "skip_queue" not in deferred_fields else None
        )
    ...
    def _update_tasks_states(
        ...
        if maximum_annotations_changed and not overlap_cohort_percentage_changed:
            ...
            tasks_with_overlap = (
                self.tasks.filter(overlap__gt=1)
        ...
        # if adding/deleting tasks and cohort settings are applied
            ...
            tasks_number_changed
            and self.overlap_cohort_percentage < 100
            and self.maximum_annotations > 1
        ):
            self._rearrange_overlap_cohort()
    ...
    def _rearrange_overlap_cohort(self):
        ...
        must_tasks = int(
            self.tasks.count() * self.overlap_cohort_percentage / 100 + 0.5
        )
        logger.info(
        ...
        tasks_with_max_annotations = all_project_tasks.annotate(
            anno=Count(
                "annotations",
                filter=Q_task_finished_annotations & Q(annotations__ground_truth=False),
            )
        ...
        logger.info(
            f"Required tasks {must_tasks} and left required tasks {left_must_tasks}"
        )
        if left_must_tasks > 0:
            ...
            self._batch_update_with_retry(
                all_project_tasks.filter(id__in=ids),
                overlap=max_annotations,
                is_labeled=True,
            )
            # order other tasks by count(annotations)
            tasks_with_min_annotations = (
                tasks_with_min_annotations.annotate(anno=Count("annotations"))
                .order_by("-anno")
                .distinct()
            )
            ...
            ids = list(
                tasks_with_min_annotations[:left_must_tasks].values_list(
                    "id", flat=True
                )
            )
            self._batch_update_with_retry(
                all_project_tasks.filter(id__in=ids), overlap=max_annotations
            )
            # assign 1 to left
            ids = list(
                tasks_with_min_annotations[left_must_tasks:].values_list(
            ...
            self._batch_update_with_retry(
                all_project_tasks.filter(id__in=ids), overlap=1
            )
    ...
    def validate_config(self, config_string, strict=False):
        ...
        with transaction.atomic():
            ...
            if self.num_tasks == 0:
                logger.debug(
                    f"Project {self} has no tasks: nothing to validate here. Ensure project summary is empty"
                )
                summary.reset()
                return
        ...
        created_labels = merge_labels_counters(
            self.summary.created_labels, self.summary.created_labels_drafts
        )
        ...
        for control_tag_from_data, labels_from_data in created_labels.items():
            ...
            if (
                ...
                and not check_control_in_config_by_regex(
                    config_string, control_tag_from_data
                )
            ):
                raise ValidationError(
            ...
            labels_from_config_by_tag = set(
                labels_from_config[
                    get_original_fromname_by_regex(config_string, control_tag_from_data)
                ]
            )
            ...
            if "Taxonomy" in tag_types:
                custom_tags = Label.objects.filter(links__project=self).values_list(
                    "value", flat=True
                )
                flat_custom_tags = set(
                    [item for sublist in custom_tags for item in sublist]
                )
            ...
            if not set(labels_from_data).issubset(set(labels_from_config_by_tag)):
                ...
                for label in different_labels:
                    annotation_label_count = self.summary.created_labels.get(
                        control_tag_from_data, {}
                    ).get(label, 0)
                    draft_label_count = self.summary.created_labels_drafts.get(
                        control_tag_from_data, {}
                    ).get(label, 0)
                    annotation_display_count = display_count(
                        annotation_label_count, "annotation"
                    )
                    draft_display_count = display_count(draft_label_count, "draft")

                    display = [
                        disp
                        for disp in [annotation_display_count, draft_display_count]
                        if disp
                    ]
                    if display:
                ...
                if (strict is True) and (
                    ...
                    and (
                        not check_control_in_config_by_regex(
                            config_string,
                            control_tag_from_data,
                            filter=dynamic_label_from_config.keys(),
                        )
                    )
                ):
                    ...
                    logger.info(
                        f"project_id={self.id} inconsistent labels in config and annotations: {diff_str}"
                    )
    ...
    def get_updated_weights(self):
        ...
        def get_label(label):
            label_value = (
                self.control_weights.get(control_name, {}).get("labels", {}).get(label)
            )
    ...
    def save(self, *args, update_fields=None, recalc=True, **kwargs):
        ...
        if self.label_config and (
            self._label_config_has_changed() or not exists or not self.control_weights
        ):
        ...
        if label_config_has_changed:
            ...
            if self.num_tasks > 0:
                logger.debug(
                    f"Sending post_label_config_and_import_tasks signal for project {self.id}"
                )
                ProjectSignals.post_label_config_and_import_tasks.send(
                    sender=Project, project=self
                )
            else:
                logger.debug(
        ...
        if recalc:
            self.update_tasks_states(
                ...
                != self.maximum_annotations,
                overlap_cohort_percentage_changed=self.__overlap_cohort_percentage
                != self.overlap_cohort_percentage,
        ...
        if self.__skip_queue != self.skip_queue:
            bulk_update_stats_project_tasks(
                self.tasks.filter(
                    Q(annotations__isnull=False) & Q(annotations__ground_truth=False)
                )
            )
    ...
    def has_annotations(self):
        ...
        return (
            Annotation.objects.filter(Q(project=self) & Q(ground_truth=False)).count()
            > 0
        )
    ...
    def eta(self):
        ...
        annotations_remain = (
            total_annotations_needed
            - min_n_finished_annotations
            - annotations_unfinished_tasks
        )
        ...
        avg_lead_time = finished_annotations.aggregate(avg_lead_time=Avg("lead_time"))[
            "avg_lead_time"
        ]
    ...
    def annotations_lead_time(self):
        annotations = Annotation.objects.filter(
            Q(project=self.id) & Q(ground_truth=False)
        )
    ...
    def get_parsed_config(self):
        if self.parsed_label_config is None:
            try:
                ...
                logger.error(
                    f"Error parsing label config for project {self.id}: {e}",
                    exc_info=True,
                )
                return {}
    ...
    def _update_tasks_counters_and_is_labeled(self, task_ids, from_scratch=True):
        ...
        while task_ids_slice := task_ids[
            page_idx * settings.BATCH_SIZE : (page_idx + 1) * settings.BATCH_SIZE
        ]:
            with transaction.atomic():
...
class ProjectOnboarding(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            ProjectOnboarding.objects.filter(
                project=self.project, finished=True
            ).count()
            == 4
        ):
...
class ProjectSummary(models.Model):
    ...
    def _get_labels(self, result):
        ...
        if (
            not result_value
            or not isinstance(result_value, list)
            or result_type == "text"
        ):
    ...
    def remove_created_annotations_and_labels(self, annotations):
        ...
        created_annotations, created_labels = (
            ...
            else (dict(self.created_annotations), dict(self.created_labels))
        )
    ...
    def remove_created_drafts_and_labels(self, drafts):
        ...
        remove_all_drafts = AnnotationDraft.objects.filter(
...
class ProjectImport(models.Model):
    ...
    status = models.CharField(
        max_length=64, choices=Status.choices, default=Status.CREATED
    )
    url = models.CharField(max_length=2048, null=True, blank=True)
    traceback = models.TextField(null=True, blank=True)
    error = models.TextField(null=True, blank=True)
    created_at = models.DateTimeField(
        _("created at"), null=True, auto_now_add=True, help_text="Creation time"
    )
    updated_at = models.DateTimeField(
        _("updated at"), null=True, auto_now_add=True, help_text="Updated time"
    )
    finished_at = models.DateTimeField(
        _("finished at"), help_text="Complete or fail time", null=True, default=None
    )
