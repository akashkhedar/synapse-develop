...
from synapse.core.argparser import parse_input_args
from synapse.core.utils.params import get_env
...
def _create_user(input_args, config):
    ...
    try:
        ...
        if token and len(token) > 5:
            ...
            print(
                f"Token {token} is not applied to user {DEFAULT_USERNAME} "
    ...
    if not org:
        org = Organization.create_organization(
            created_by=user,
            title="Synapse",
            legacy_api_tokens_enabled=input_args.enable_legacy_api_token,
        )
    else:
        org.add_user(user)
...
def _init(input_args, config):
    ...
    if (
        user
        and input_args.project_name
        and not _project_exists(input_args.project_name)
    ):
        from projects.models import Project
...
def main():
    ...
    if input_args.data_dir:
        ...
        os.environ.setdefault("SYNAPSE_BASE_DATA_DIR", str(data_dir_path.absolute()))
    ...
    if not get_env("HOST"):
        os.environ.setdefault(
            "HOST", host
        )  # it will be passed to settings.HOSTNAME as env var
    ...
    # init
        ...
        print("")
        print("Synapse has been successfully initialized.")
        if input_args.command != "start" and input_args.project_name:
            print("Start the server: synapse start " + input_args.project_name)
            return

    # start with migrations from old projects, '.' project_name means 'synapse start' without project name
    elif input_args.command == "start" and input_args.project_name != ".":
        from projects.models import Project

        from synapse.core.old_ls_migration import migrate_existing_project
        ...
        if input_args.project_name and not _project_exists(input_args.project_name):
            ...
            if project_path.exists():
                print("Project directory from previous version of synapse found")
                ...
                print(
                    Fore.LIGHTYELLOW_EX
                    + "\n*** WARNING! ***\n"
                    + f"Project {input_args.project_name} migrated to Synapse Database\n"
            ...
            if not migrated:
                print(
                    ...
                    "Did you miss create it first with `synapse init {project_name}` ?".format(
                        project_name=input_args.project_name
                    )
                )
                return

    # on `start` command, launch browser if --no-browser is not specified and start Synapse server
    if input_args.command == "start" or input_args.command is None:
        from synapse.core.utils.common import start_browser
        ...
        if cert_file or key_file:
            logger.error(
                "Synapse doesn't support SSL web server with cert and key.\n"
                "Use nginx or other servers for it."
            )
            return

        # internal port and internal host for server start
        internal_host = input_args.internal_host or config.get(
        ...
        try:
            ...
            logger.warning(
                f"Can't parse PORT '{internal_port}': {e}; default value 8080 will be used"
            )
            internal_port = 8080
