...
from organizations.permissions import check_annotation_permission
...
@method_decorator(
    ...
    decorator=(
        extend_schema(
            tags=["Tasks"],
            summary="Create task",
            description="Create a new labeling task in Synapse.",
            request={
    ...
    """,
            parameters=[
                ...
                OpenApiParameter(
                    ...
                    "[Read more about available filters](https://synapse.io/sdk/data_manager.html)<br/>"
...
@method_decorator(
    ...
    decorator=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="id", type=OpenApiTypes.STR, location="path", description="Task ID"
            ),
        ],
        request=None,
        responses={
            "200": OpenApiResponse(
                ...
                examples=[
                    OpenApiExample(
                        name="response",
                        value=dm_task_response_example,
                        media_type="application/json",
                    )
                ],
            )
...
@method_decorator(
    ...
    decorator=extend_schema(
        tags=["Tasks"],
        summary="Delete task",
        description="Delete a task in Synapse. This action cannot be undone!",
        parameters=[
            OpenApiParameter(
                name="id", type=OpenApiTypes.STR, location="path", description="Task ID"
            ),
...
class TaskAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        if review:
            ...
            if flag_set(
                "fflag_fix_back_bros_182_api_task_optimizations", user=self.request.user
            ):
                kwargs = {
                    "all_fields": True,
        ...
        responses={
            "200": OpenApiResponse(
                ...
                examples=[
                    OpenApiExample(
                        name="response",
                        value=annotation_response_example,
                        media_type="application/json",
                    )
                ],
            )
...
class AnnotationAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        """
        Override to add annotation visibility check.
        ...
        try:
            ...
            visible_annotations = AnnotationWorkflowService.get_visible_annotations(
                user, obj.task, include_own=True
            )
            if not visible_annotations.filter(id=obj.id).exists():
    ...
    def update(self, request, *args, **kwargs):
        # Check annotation permission - org members cannot annotate
        annotation = self.get_object()
        check_annotation_permission(request.user, task=annotation.task)
        
        # =====================================================================
        # OWNERSHIP CHECK - Annotators can only update their own annotations
        # =====================================================================
        try:
            from annotators.annotation_workflow import AnnotationWorkflowService
            can_update, reason = AnnotationWorkflowService.can_update_annotation(
    ...
    def delete(self, request, *args, **kwargs):
        # Check annotation permission - org members cannot delete annotations
        annotation = self.get_object()
        check_annotation_permission(request.user, task=annotation.task)
...
@method_decorator(
    ...
    decorator=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="id", type=OpenApiTypes.INT, location="path", description="Task ID"
            ),
        ],
        request=None,
        responses={
...
class AnnotationsListAPI(GetParentObjectMixin, generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # Experts can see all annotations for review
        # Clients can only see after expert review is complete
        try:
            from annotators.annotation_workflow import AnnotationWorkflowService

            visible_annotations = AnnotationWorkflowService.get_visible_annotations(
                self.request.user, task, include_own=True
            )
            return visible_annotations.order_by("pk")
        except ImportError:
            pass  # Module not available, use default
        except Exception as e:
            logger.warning(f"Error in annotation visibility filtering: {e}")
    ...
    def perform_create(self, ser):
        ...
        # Check annotation permission - org members cannot create annotations
        check_annotation_permission(self.request.user, task=task)

        # annotator has write access only to annotations and it can't be checked it after serializer.save()
        user = self.request.user

        # =====================================================================
        # DUPLICATE ANNOTATION PREVENTION FOR ANNOTATORS
        # =====================================================================
        # Check if annotator can create a new annotation using the workflow service
        try:
            from annotators.annotation_workflow import AnnotationWorkflowService

            can_create, reason = AnnotationWorkflowService.can_create_annotation(
                user, task
            )
            if not can_create:
        ...
        # =====================================================================
        # TRIGGER CONSOLIDATION WORKFLOW AFTER ANNOTATION CREATION
        # =====================================================================
        # This handles:
        # - Updating task assignment status
        # - Checking if all annotators have completed
        # - Running consolidation algorithm
        # - Creating expert review tasks if needed
        try:
            from annotators.annotation_workflow import AnnotationWorkflowService

            AnnotationWorkflowService.on_annotation_created(annotation)
        except Exception as e:
            logger.warning(f"Error in annotation workflow callback: {e}")
            # Don't fail the annotation creation if workflow fails
...
class AnnotationDraftListAPI(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        # Check annotation permission - org members cannot create drafts
        task = Task.objects.get(pk=task_id)
        check_annotation_permission(user, task=task)
...
class AnnotationDraftAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        # Check annotation permission - org members cannot update drafts
        draft = self.get_object()
        check_annotation_permission(request.user, task=draft.task)
        return super().update(request, *args, **kwargs)

    def destroy(self, request, *args, **kwargs):
        # Check annotation permission - org members cannot delete drafts
        draft = self.get_object()
        check_annotation_permission(request.user, task=draft.task)
        ...
        responses={
            "200": OpenApiResponse(
                ...
                examples=[
                    OpenApiExample(
                        name="response",
                        value=[prediction_response_example],
                        media_type="application/json",
                    )
                ],
            )
