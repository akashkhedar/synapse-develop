...
from synapse_sdk.synapse_interface.objects import PredictionValue
...
class Task(TaskMixin, FsmHistoryStateModel):
    ...
    data = JSONField(
        ...
        "on the Import page in the Synapse Data Manager UI.",
    )
    ...
    target_assignment_count = models.IntegerField(
        default=3, help_text="Target number of assignments for consensus (default: 3)"
    )
        ...
        if self.project.skip_queue == SkipQueue.IGNORE_SKIPPED:
            ...
            if self.project.skip_queue == SkipQueue.REQUEUE_FOR_ME:
                ...
                raise ValidationError(
                    f"Invalid SkipQueue value: {self.project.skip_queue}"
                )
    ...
    def has_lock(self, user=None):
        ...
        if self.project.show_ground_truth_first:
            ...
            # https://Synapse.atlassian.net/browse/LEAP-1963
    ...
    def overlap_with_agreement_threshold(self, num, num_locks):
        ...
        if (
            ...
            if (
                ...
                and agreement < self.project.lse_project.agreement_threshold
            ):
                return (
                    min(
                        self.overlap
                        + self.project.lse_project.max_additional_annotators_assignable,
                        num + 1,
                    )
                    if num_locks == 0
                    else num
    ...
    def set_lock(self, user):
        ...
        if num_locks < self.overlap:
            ...
            if (
                flag_set(
                    "fflag_feat_all_leap_1534_custom_task_lock_timeout_short", user=user
                )
    ...
    def resolve_uri(self, task_data, project):
        ...
        if project.task_data_login and project.task_data_password:
            ...
            for field in task_data:
                ...
                storage = (
                    get_storage_by_url(task_data[field], storage_objects)
                    or self.storage
                )
                if storage:
                    try:
    ...
    def save(self, *args, update_fields=None, **kwargs):
        # Enforce 3-annotator consensus by default (always use 3, not 1)
        if self.pk is None and self.project:
            required_overlap = getattr(self.project, "required_overlap", 3)
            # Always ensure at least 3 for consensus workflow
            required_overlap = max(3, required_overlap)

            # Set overlap and target to project's required_overlap (minimum 3)
            self.overlap = required_overlap
            self.target_assignment_count = required_overlap

            if update_fields is not None:
                update_fields = {"overlap", "target_assignment_count"}.union(
                    update_fields
                )

        if self.inner_id == 0:
            task = (
                Task.objects.filter(project=self.project).order_by("-inner_id").first()
            )
            max_inner_id = 1
            if task:
...
class TaskLock(FsmHistoryStateModel):
    ...
    created_at = models.DateTimeField(
        _("created at"), auto_now_add=True, help_text="Creation time", null=True
    )
...
class Prediction(models.Model):
    ...
    def save(self, *args, update_fields=None, **kwargs):
        if self.project_id is None and self.task_id:
            logger.warning(
                "project_id is not set for prediction, project_id being set in save method"
            )
            self.project_id = (
                Task.objects.only("project_id").get(pk=self.task_id).project_id
            )
            if update_fields is not None:
        ...
        # set updated_at field of task to now()
        self.update_task()
        return super(Prediction, self).save(
    ...
    def create_no_commit(
        ...
        try:
            ...
            # given the data receive, create annotation regions in Synapse format
            # e.g. {"sentiment": "positive"} -> {"value": {"choices": ["positive"]}, "from_name": "", "to_name": "", ..}
            pred = PredictionValue(
                result=label_interface.create_regions(data)
            ).model_dump()
...
class FailedPrediction(models.Model):
    ...
    task = models.ForeignKey(
        "tasks.Task", on_delete=models.CASCADE, related_name="failed_predictions"
    )
    created_at = models.DateTimeField(_("created at"), auto_now_add=True)
...
class PredictionMeta(models.Model):
    ...
    prompt_cost = models.FloatField(
        _("prompt cost"), null=True, blank=True, help_text=_("Cost of the prompt")
    )
    ...
    total_cost = models.FloatField(
        _("total cost"), null=True, blank=True, help_text=_("Total cost")
    )
    ...
    @classmethod
    def create_no_commit(
        cls, data, prediction: Union[Prediction, FailedPrediction]
    ) -> Optional["PredictionMeta"]:
        """
        ...
        try:
            prediction_meta = PredictionMeta(
                ...
                total_tokens_count=data.get("prompt_tokens", 0)
            ...
            logger.error(
                f"Error creating prediction meta with {data}: {exc}. Traceback: {traceback.format_exc()}"
            )
...
def deprecated_bulk_update_stats_project_tasks(tasks, project=None):
    ...
    with transaction.atomic():
        ...
        if use_overlap:
            ...
            finished_q = Q(
                GreaterThanOrEqual(completed_annotations_f_expr, F("overlap"))
            )
