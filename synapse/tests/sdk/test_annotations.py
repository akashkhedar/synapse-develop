...
from synapse_sdk.data_manager import Column, Filters, Operator, Type
from synapse_sdk.synapse_interface import LabelInterface
from synapse_sdk.synapse_interface.create import labels
from synapse_sdk.synapse_interface.objects import AnnotationValue, TaskValue
...
def test_annotations_CRUD(django_live_url, business_client):
    ...
    client.projects.import_tasks(id=p.id, request=[task_data.model_dump()])
...
def test_annotation_marks_task_as_labeled(django_live_url, business_client):
    ...
    client.projects.import_tasks(id=p.id, request=[task.model_dump() for task in task_data])
    ...
    task_id = tasks[0].id
    project = client.projects.get(p.id)
