...
from synapse.tests.sdk.utils import sdk_logs
...
def test_export_tasks(django_live_url, business_client):
    ...
    task_data = [{'data': {'my_text': 'Test task ' + str(i)}} for i in range(10)]
    client.projects.import_tasks(id=p.id, request=task_data)
...
def test_upload_and_list_tasks_does_not_log_to_stderr(django_live_url, business_client, caplog):
    ...
    client = Synapse(base_url=django_live_url, api_key=business_client.api_key)
    p = client.projects.create(title='New Project', label_config=LABEL_CONFIG_AND_TASKS['label_config'])
    client.projects.import_tasks(id=p.id, request=LABEL_CONFIG_AND_TASKS['tasks_for_import'])
