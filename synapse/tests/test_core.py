...
from core.utils.exceptions import InvalidUploadUrlError, SynapseAPIException
...
def test_user_info(business_client):
    from synapse.server import _create_user, _get_user_info
...
@pytest.mark.parametrize(
    'command_line, result',
    [
        (['synapse', 'user', '--username', 'test@test.com', '--password', '12345678'], None),
    ],
)
@pytest.mark.django_db
...
def test_string_is_url():
    from synapse.core.utils.common import string_is_url
...
def test_get_client_ip():
    from synapse.core.utils.common import get_client_ip
...
def test_timestamp_now():
    from synapse.core.utils.common import timestamp_now
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('http://example', False, SynapseAPIException),
        ('http://example', True, SynapseAPIException),
        # localhost
