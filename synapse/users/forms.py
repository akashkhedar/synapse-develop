...
USERNAME_LENGTH_ERR = (
    f"Please enter a username {USERNAME_MAX_LENGTH} characters or fewer in length"
)
DISPLAY_NAME_LENGTH_ERR = (
    f"Please enter a display name {DISPLAY_NAME_LENGTH} characters or fewer in length"
)
...
class LoginForm(forms.Form):
    ...
    email = (
        forms.CharField(label="User")
        if settings.USE_USERNAME_FOR_LOGIN
...
class UserSignupForm(forms.Form):
    email = forms.EmailField(
        label="Work Email", error_messages={"required": "Invalid email"}
    )
    ...
    user_role = forms.ChoiceField(
        choices=[("client", "Client"), ("annotator", "Annotator")],
        required=True,
        error_messages={"required": "Please select a user role"},
    )
