...
class UserLastActivityMixin(models.Model):
    ...
    def update_last_activity(self):
        ...
        if flag_set(
            "fflag_fix_back_plt_840_redis_last_activity_29072025_short", user="auto"
        ):
...
class User(UserMixin, AbstractBaseUser, PermissionsMixin, UserLastActivityMixin):
    ...
    email_verified_at = models.DateTimeField(
        ...
        null=True,
        blank=True,
        help_text=_("Timestamp when the email was verified."),
    )
    ...
    is_client = models.BooleanField(
        _("client role"),
        default=False,
        help_text=_("Designates whether the user has client privileges."),
    )

    is_annotator = models.BooleanField(
        _("annotator role"),
        default=False,
        help_text=_("Designates whether the user has annotator privileges."),
    )
    ...
    is_expert = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Designates whether the user has expert privileges for quality review."
        ),
    )

    # Annotator status for test workflow
    annotator_status = models.CharField(
        _("annotator status"),
        max_length=32,
        default="pending_verification",
        help_text=_("Status of annotator in the qualification workflow"),
    )
    ...
    def get_user_role(self):
        """Return the user's primary role as a string"""
        if self.is_client:
            return "client"
    ...
    def has_role(self, role):
        ...
        role_map = {
            ...
            "annotator": self.is_annotator,
            "expert": self.is_expert,
        }
        return role_map.get(role, False)

    @property
    def is_only_annotator(self):
        """Check if user is ONLY an annotator (not client or expert)"""
        return self.is_annotator and not self.is_client and not self.is_expert

    @property
    def is_only_client(self):
        """Check if user is ONLY a client (not annotator or expert)"""
        return self.is_client and not self.is_annotator and not self.is_expert

    @property
    def is_only_expert(self):
        """Check if user is ONLY an expert (not client or annotator)"""
        return self.is_expert and not self.is_client and not self.is_annotator
