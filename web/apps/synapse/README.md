...
The Synapse App is the cornerstone of the frontend aspect of Synapse. This React-based application is where the magic of frontend development for Synapse occurs. In this app, developers have the freedom and capability to create new pages and determine how libraries are utilized within the Synapse environment. It's a purely frontend module, dedicated to crafting and refining the user interface and user experience aspects of Synapse.
...
- **`yarn ls:watch`: Build SF continuously**
    - Automatically builds the Synapse on every change, providing a real-time development experience.
...
To create a page using Django is simple and straightforward: select an app within `synapse/` directory, add a url and create a view with a html template. React app will handle it automatically if there's no React page for a particular route.
