...
const loadAsyncPage = async (url) => {
  try {
    ...
    if (response.status === 401) {
      // Only redirect to root if not already there to prevent infinite loop
      const currentPath = window.location.pathname;
      const publicPaths = ['/', '/login', '/signup-client', '/register-annotator', '/annotators', '/services', '/about', '/contact', '/security', '/careers'];
...
export const AsyncPage = ({ children }) => {
  ...
  const onMessage = useCallback((event) => {
    ...
    if (event.data?.source !== "synapse") return;
  ...
  useEffect(() => {
    // Don't set up async page handlers for public routes
    const publicPaths = ['/', '/login', '/signup-client', '/register-annotator', '/services', '/about', '/contact', '/security', '/careers', '/blog'];
