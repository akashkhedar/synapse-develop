...
import { IconBell } from "@synapse/icons";
...
export const VersionProvider = ({ children }) => {
  ...
  const fetchVersion = useCallback(async () => {
    ...
    if (response !== null) {
      const data = response["synapse-os-package"];
...
export const VersionNotifier = ({ showNewVersion, showCurrentVersion }) => {
  ...
  const url = `https://synapse.io/redirect/update?version=${version}`;
  ...
  return newVersion && showNewVersion ? (
    <li className={cn("version-notifier").toClassName()}>
      <a href={url} target="_blank" rel="noreferrer">
        ...
        <div className={cn("version-notifier").elem("content").toClassName()}>
          <div
            className={cn("version-notifier").elem("title").toClassName()}
            data-date={updateTime}
          >
            {latestVersion} Available
          </div>
          <div
            className={cn("version-notifier").elem("description").toClassName()}
          >
    ...
    <Link
      className={cn("current-version").toClassName()}
      to="/version"
      target="_blank"
    >
      v{version}
