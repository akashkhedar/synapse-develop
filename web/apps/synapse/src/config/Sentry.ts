...
// Provide default APP_SETTINGS for dev mode when running without Django
...
const setTags = () => {
  ...
  if (APP_SETTINGS.user && APP_SETTINGS.user.email) {
    Sentry.setUser({
...
const getVersion = () => {
  const version = APP_SETTINGS.version?.["synapse-os-package"]?.version;
