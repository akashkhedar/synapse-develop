...
const LEVEL_CONFIG: Record<string, { color: string; icon: string; next: string | null; tasksReq: number; accuracyReq: number }> = {
  new: { color: "#6b7280", icon: "◇", next: "junior", tasksReq: 0, accuracyReq: 0 },
  junior: { color: "#22c55e", icon: "◆", next: "regular", tasksReq: 50, accuracyReq: 70 },
  regular: { color: "#3b82f6", icon: "★", next: "senior", tasksReq: 200, accuracyReq: 80 },
  senior: { color: "#a855f7", icon: "✦", next: "expert", tasksReq: 500, accuracyReq: 90 },
  expert: { color: "#f59e0b", icon: "✧", next: null, tasksReq: 1000, accuracyReq: 95 },
};
...
export const EarningsDashboard: React.FC = () => {
  ...
  const levelProgress = useMemo(() => {
    if (!summary) return { percentage: 0, nextLevel: null, tasksNeeded: 0, accuracyNeeded: 0 };
    ...
    if (!nextLevel) return { percentage: 100, nextLevel: null, tasksNeeded: 0, accuracyNeeded: 0 };
    
    const nextConfig = LEVEL_CONFIG[nextLevel];
    const taskProgress = Math.min(100, (summary.trust_level.tasks_completed / nextConfig.tasksReq) * 100);
    const accuracyProgress = Math.min(100, (summary.trust_level.accuracy_score / nextConfig.accuracyReq) * 100);
    
    return {
      percentage: Math.min(taskProgress, accuracyProgress),
      nextLevel,
      tasksNeeded: Math.max(0, nextConfig.tasksReq - summary.trust_level.tasks_completed),
  ...
  const calendarData = useMemo(() => {
    ...
    const days: { date: Date; isActive: boolean; isToday: boolean; inMonth: boolean }[] = [];
  ...
  const chartData = useMemo(() => {
    ...
    const avg = earnings.reduce((sum, e) => sum + e.amount, 0) / earnings.length;
    ...
    return (
      <div className="earnings-page">
        <div className="earnings-error">
          ...
          <p className="error-desc">Something went wrong while fetching your data.</p>
          <motion.button
  ...
  const levelConfig = LEVEL_CONFIG[summary.trust_level.level];
  const currentLevelIndex = LEVEL_ORDER.indexOf(summary.trust_level.level);
      ...
      <div className="earnings-container">
        ...
        >
          <div className="header-left">
            ...
            <p className="header-subtitle">Track your performance and revenue</p>
          </div>
          <motion.button
        ...
        >
          <div className="stat-card available">
            ...
            <div className="stat-value primary">{formatCurrency(summary.available_balance)}</div>
            <div className="stat-hint">Ready to withdraw</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">PENDING</div>
            <div className="stat-value">{formatCurrency(summary.pending_approval)}</div>
            <div className="stat-hint">Awaiting approval</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">TOTAL EARNED</div>
            <div className="stat-value">{formatCurrency(summary.total_earned)}</div>
            <div className="stat-hint">Lifetime earnings</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">TASKS COMPLETED</div>
            <div className="stat-value">{summary.total_tasks.toLocaleString()}</div>
            ...
            >
              ...
              <div className="trust-content">
                <div className="trust-main">
                  <div className="level-display">
                    <span className="level-icon" style={{ color: levelConfig.color }}>{levelConfig.icon}</span>
                    <div className="level-info">
                      <span className="level-name">{summary.trust_level.level.toUpperCase()}</span>
                      <span className="level-multiplier">{summary.trust_level.multiplier}x earnings multiplier</span>
                    </div>
                  </div>
                  ...
                  {levelProgress.nextLevel && (
                    <div className="progress-section">
                      <div className="progress-meta">
                        <span>Progress to {levelProgress.nextLevel.toUpperCase()}</span>
                      ...
                      <p className="progress-hint">
                        {levelProgress.tasksNeeded > 0 && `${levelProgress.tasksNeeded} more tasks needed`}
                        {levelProgress.tasksNeeded > 0 && summary.trust_level.accuracy_score < levelProgress.accuracyNeeded && ' • '}
                        {summary.trust_level.accuracy_score < levelProgress.accuracyNeeded && `${levelProgress.accuracyNeeded}% accuracy required`}
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  {LEVEL_ORDER.map((level, index) => {
                    ...
                    return (
                      <React.Fragment key={level}>
                        <div className={`level-node ${isCurrent ? 'current' : ''} ${isPast ? 'past' : ''}`}>
                          <span className="node-icon" style={{ color: isPast || isCurrent ? config.color : '#333' }}>
                            {config.icon}
                          </span>
                          <span className="node-label">{level}</span>
                        </div>
                        {index < LEVEL_ORDER.length - 1 && (
                          <div className={`level-connector ${isPast ? 'filled' : ''}`} />
                        )}
                      </React.Fragment>
                    );
                  })}
              ...
              <div className="trust-stats">
                <div className="trust-stat">
                  <span className="stat-num">{summary.trust_level.tasks_completed}</span>
                  <span className="stat-lbl">Tasks</span>
                </div>
                <div className="trust-stat">
                  <span className="stat-num">{summary.trust_level.accuracy_score.toFixed(1)}%</span>
                  <span className="stat-lbl">Accuracy</span>
                </div>
                <div className="trust-stat">
                  <span className="stat-num">{summary.trust_level.honeypot_pass_rate.toFixed(0)}%</span>
                  <span className="stat-lbl">Quality Rate</span>
                </div>
                <div className="trust-stat">
                  <span className={`stat-num ${summary.trust_level.fraud_flags > 0 ? 'warning' : ''}`}>
              ...
              {summary.trust_level.fraud_flags > 0 && (
                <div className="fraud-warning">
                  ...
                  <span>{summary.trust_level.fraud_flags} quality flag(s) detected. Maintain high accuracy to remove.</span>
                </div>
              )}
            </motion.section>
            ...
            >
              ...
              <div className="chart-wrapper">
                ...
                <div className="bar-chart">
                  ...
                  <div 
                    className="avg-line" 
                    style={{ bottom: `${(chartData.avg / chartData.max) * 100}%` }}
                  >
                    ...
                    {chartData.bars.map((bar, i) => (
                      ...
                      >
                        ...
                        <span className="bar-day">
                          {bar.date.toLocaleDateString('en-US', { weekday: 'narrow' })}
                        </span>
                      </motion.div>
                    ))}
                  </div>
                ...
                <div className="chart-summary">
                  <div className="summary-item">
                    ...
                    <span className="summary-value">{formatCurrency(summary.weekly_earnings)}</span>
                  </div>
                  <div className="summary-item">
                    <span className="summary-label">THIS MONTH</span>
                    <span className="summary-value">{formatCurrency(summary.monthly_earnings)}</span>
                  </div>
                  <div className="summary-item">
                    <span className="summary-label">DAILY AVG</span>
                    <span className="summary-value accent">{formatCurrency(chartData.avg)}</span>
                  </div>
                </div>
              </div>
            </motion.section>
          ...
          <div className="right-col">
            ...
            >
              <div className="section-header">
                ...
                <div className="streak-display">
                  <span className="streak-num">{summary.activity_streak || 0}</span>
                  <span className="streak-text">day streak</span>
                </div>
              </div>
              
              <div className="calendar-nav">
                <button className="nav-btn" onClick={() => setSelectedMonth(new Date(selectedMonth.getFullYear(), selectedMonth.getMonth() - 1))}>
                  ←
                </button>
                <span className="nav-month">{selectedMonth.toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</span>
                <button className="nav-btn" onClick={() => setSelectedMonth(new Date(selectedMonth.getFullYear(), selectedMonth.getMonth() + 1))}>
                  →
                </button>
              </div>
              ...
              <div className="calendar">
                ...
                <div className="calendar-body">
                  {calendarData.map((day, i) => (
                    <div 
                      key={i} 
                      className={`cal-day ${day.isActive ? 'active' : ''} ${day.isToday ? 'today' : ''} ${!day.inMonth ? 'outside' : ''}`}
                    >
                      {day.date.getDate()}
                    </div>
                  ))}
            ...
            >
              <div className="section-header">
                ...
                <button className="view-all" onClick={() => history.push("/annotators/transactions")}>
                  View all
                </button>
              </div>
              ...
              <div className="transactions-list">
                {summary.recent_transactions.length === 0 ? (
                  <div className="empty-state">
                    ...
                    <span className="empty-hint">Complete tasks to start earning</span>
                  </div>
                ) : (
            ...
            >
              ...
              <div className="multiplier-note">
                Your <span className="highlight">{summary.trust_level.multiplier}x</span> multiplier applies to all stages
              </div>
            </motion.section>
          </div>
        </div>
