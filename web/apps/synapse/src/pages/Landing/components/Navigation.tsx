...
export const Navigation = () => {
  ...
  const dropdownContent: Record<string, { title: string; items: string[] }[]> = {
    'PRODUCTS': [
      { title: '//PLATFORM', items: ['ANNOTATION STUDIO', 'QUALITY ENGINE', 'WORKFORCE'] },
      { title: '//FOR EVERY TEAM', items: ['ENTERPRISE', 'STARTUPS', 'RESEARCH'] },
      { title: '//DATA TYPES', items: ['IMAGES', 'TEXT & NLP', 'AUDIO', 'VIDEO'] },
    ],
    'RESOURCES': [
      ...
      <div className="max-w-[1400px] mx-auto px-10 lg:px-16 py-2">
        <div className="flex items-center justify-between h-20">
          ...
          >
            ...
            <span className="text-white font-medium text-xl tracking-wide">Synapse</span>
          </motion.div>
              ...
              <AnimatePresence>
                {activeDropdown === 'PRODUCTS' && (
                  <motion.div
                    ...
                    className="absolute top-full left-1/2 -translate-x-1/2 mt-6 bg-black border border-gray-800 p-8 min-w-[600px] grid grid-cols-3 gap-8"
                  >
                    {dropdownContent['PRODUCTS'].map((section, idx) => (
                      <div key={idx}>
          ...
          <div className="flex items-center gap-8 flex-shrink-0">
            ...
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="bg-[#e8e4d9] text-black px-8 py-5 text-[13px] font-semibold tracking-[0.15em] transition-all hover:bg-[#6f47cc]"
            >
              GET STARTED
            </motion.button>
          </div>
