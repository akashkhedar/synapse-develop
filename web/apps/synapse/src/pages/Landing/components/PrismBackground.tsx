...
// Performance: Target ~30fps for smooth animation without taxing GPU
const TARGET_FPS = 30;
...
const Prism = ({
  ...
  useEffect(() => {
    ...
    // Cleanup any existing renderer first (React 18 StrictMode)
    if (rendererRef.current) {
      try {
        const oldCanvas = rendererRef.current.gl.canvas;
        if (oldCanvas.parentElement === container) {
    ...
    try {
      renderer = new Renderer({
        ...
        alpha: transparent,
        antialias: false,
        powerPreference: "low-power", // Prefer integrated GPU for less heat/power
      });
    } catch (e) {
    ...
    Object.assign(gl.canvas.style, {
      ...
      contain: "strict", // CSS containment for better perf
    ...
    let lastFrameTime = 0; // FPS throttling
    ...
    const render = (t: number) => {
      // FPS throttling - skip frames to maintain target FPS
    ...
    // Performance: Suspend when offscreen
  ...
  return (
    <div 
      ...
      ref={containerRef}
      style={{
        contain: "strict", // CSS containment for scroll perf
        willChange: "transform", // Hint for GPU layer
      }}
    />
  );
};

// Export as PrismBackground for backward compatibility
export const PrismBackground = Prism;
