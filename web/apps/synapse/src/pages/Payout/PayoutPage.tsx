...
interface PayoutPageProps {
  /** Type of user: 'annotator' or 'expert' */
  userType: "annotator" | "expert";
  /** Route to redirect on back button */
  backRoute: string;
  /** Route to redirect on auth failure */
  loginRoute: string;
}
...
/**
 * Shared Payout component for both Annotators and Experts
 */
export const PayoutPage: React.FC<PayoutPageProps> = ({
  ...
  const apiBase = userType === "expert" ? "/api/annotators/expert" : "/api/annotators";
  ...
  const fetchData = useCallback(async () => {
    try {
      const [bankRes, payoutRes, earningsRes] = await Promise.all([
        fetch(`${apiBase}/bank-details`, { credentials: "include" }),
        fetch(`${apiBase}/payouts`, { credentials: "include" }),
        fetch(`${apiBase}/earnings`, { credentials: "include" }),
      ]);

      if (bankRes.status === 403 || bankRes.status === 401) {
  ...
  }, [apiBase, history, loginRoute, toast]);
  ...
  const formatCurrency = (amount: number) => {
    return formatCurrencyUtil(amount, "INR", "en-IN");
  };
  ...
  const saveBankDetails = async () => {
    try {
      const response = await fetch(`${apiBase}/bank-details`, {
        method: "POST",
        credentials: "include",
  ...
  const cancelPayout = async (payoutId: number) => {
    ...
    try {
      const response = await fetch(
        `${apiBase}/payouts/${payoutId}/cancel`,
        {
          method: "POST",
          credentials: "include",
        }
  ...
  // Get pending amount based on user type
  const pendingAmount = earnings?.pending_approval ?? earnings?.pending_payout ?? 0;
