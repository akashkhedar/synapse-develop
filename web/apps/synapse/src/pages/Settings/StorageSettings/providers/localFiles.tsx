...
import { IconFolderOpen } from "@synapse/icons";
import {
  Alert,
  AlertDescription,
  AlertTitle,
} from "@synapse/shad/components/ui/alert";

const localFilesDocumentRoot =
  typeof window === "undefined"
    ? undefined
    : window.APP_SETTINGS?.local_files_document_root;
const localFilesServingEnabled =
  typeof window === "undefined"
    ? true
    : window.APP_SETTINGS?.local_files_serving_enabled !== false;
const isCommunityEdition =
  typeof window === "undefined"
    ? false
    : window.APP_SETTINGS?.version?.edition === "Community";
  ...
  return (
    <>
      <Alert variant="destructive">
        ...
        <AlertDescription>
          Set the "LOCAL_FILES_SERVING_ENABLED" environment variable to "true"
          and restart Synapse to enable Local Files storage. See the
          documentation for details:{" "}
          <a
            href="https://synapse.io/guide/storage.html#Local-storage"
            target="_blank"
            rel="noreferrer"
          >
          ...
          {isCommunityEdition && (
            <Alert variant="info">
              <AlertDescription>
                <p>
                  Tip: Create a "mydata" or "synapse-data" directory next to the
                  command you use to run Synapse Studio and local file serving
                  will be enabled automatically.
                </p>
                <p>
                  If you run the Docker image, the app starts in "/synapse", so
                  you can bind-mount your host folder to "/synapse/mydata" or
                  "/synapse/synapse-data" inside the container to enable local
...
export const localFilesProvider: ProviderConfig = {
  ...
  title: "Local Files",
  description:
    "Configure your local file storage connection with all required Synapse settings",
  icon: () => (
    <IconFolderOpen
      width={40}
      height={40}
  ...
  fields: [
    ...
    },
    {
      ...
      description: `This path must be an absolute path on the host machine where Synapse is running and start with \n"${localFilesDocumentRoot}" (LOCAL_FILES_DOCUMENT_ROOT).`,
    },
  ],
