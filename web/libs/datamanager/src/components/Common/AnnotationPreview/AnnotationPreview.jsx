...
class PreviewGenerator {
  ...
  constructor(labelingConfig) {
    ...
    this.sf = new window.Synapse(this.root, {
      user: { id: 1 },
      interfaces: [],
  ...
  async processJob() {
    ...
    this.sf.initializeStore(taskToSFFormat(task));
    this.sf.annotationStore.selectAnnotation(annotation.pk ?? annotation.id);
  ...
  async createPreviews(attempts) {
    ...
    try {
      return this.sf.annotationStore.selected.generatePreviews();
    } catch (err) {
      await wait(1000);
...
export const AnnotationPreview = injector(
  observer(({ labelingConfig, name, task, annotation, style, ...props }) => {
    ...
    return preview ? (
      ...
      <div
        className={cn("annotation-preview").toString()}
        width={props.width}
        height={props.height}
      >
        <Spinner
