...
import { IconTimelinePause, IconTimelinePlay } from "@synapse/icons";
...
export const MediaPlayer = ({ src, video = false }) => {
  ...
  return enabled ? (
    <div
      className={cn("player").mod({ video }).toClassName()}
      ...
      {showError ? (
        ...
        <div className={cn("player").elem("playback").toClassName()}>
          <Space className={cn("player").elem("controls").toClassName()} spread>
            <Space size="small">
              <div
                className={cn("player").elem("play").toClassName()}
                onClick={togglePlay}
              >
                {state.playing ? <IconTimelinePause /> : <IconTimelinePlay />}
              </div>
              {!video && (
                <div className={cn("player").elem("track").toClassName()}>
                  {filename(src)}
                </div>
              )}
            </Space>
            <Space
              className={cn("player").elem("time").toClassName()}
              size="small"
            >
