...
const CellRenderer = observer(
  ({ col: colInput, data, decoration, cellViews }) => {
    ...
    if (Cell instanceof Function) {
      ...
      return (
        <span
          className={tableCN.elem("cell").mix(cellClassName).toString()}
          {...rest}
          key={id}
        >
          <Cell data={data} />
    ...
    // DEBUG: Log for image columns
    if (altType === 'Image' || valuePath.includes('image')) {
      console.log('[CellRenderer] Image column:', {
        taskId: data?.id,
        columnId: id,
        valuePath,
        altType,
        ...
        dataIsMap: data.data instanceof Map,
      });
    }
  ...
  ({
    ...
    React.useEffect(() => {
      try {
        ...
        console.error("[TableRow] render log failed:", e);
      }
    }, [data, columns]);
