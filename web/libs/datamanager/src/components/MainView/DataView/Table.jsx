import { IconQuestionOutline } from "@synapse/icons";
...
const injector = inject(({ store }) => {
  ...
  const props = {
    ...
    canAnnotate: store.SDK?.canAnnotate !== false, // Check if user can annotate
  };
...
export const DataView = injector(
  ({
    ...
    const storeListLen = dataStore?.list?.length ?? 0;
    const colsLen = columns?.length ?? 0;
    const hiddenLen = (hiddenColumns && hiddenColumns.length) || 0;

    useEffect(() => {
      try {
        console.log("[DataView] debug", {
          dataLen,
          storeListLen,
          total,
          colsLen,
        ...
        console.error("[DataView] debug log failed:", e);
      }
    }, [dataLen, storeListLen, total, colsLen, hiddenLen, view?.id]);
    ...
    const onRowClick = useCallback(
      async (item, e) => {
        ...
        if (store.SDK.type === "DE") {
          ...
          let rootStore;
          try {
            rootStore = getRoot(view);
          } catch (err) {
            console.warn(
              "[onRowClick] MST node is not alive, skipping startLabeling"
            );
            return;
          }
          if (!rootStore || rootStore.isAlive === false) {
            console.warn(
              "[onRowClick] MST root store is not alive, skipping startLabeling"
            );
            return;
          }
          rootStore.startLabeling(item);
        }
      },
      [view, columns, canAnnotate]
    );
          ...
          return (
            <div className={cn("no-results").toClassName()}>
              <EmptyState
                ...
                canLabel={canLabel && canAnnotate} // Only allow labeling if user can annotate
                canAnnotate={canAnnotate} // Pass canAnnotate to EmptyState
                onLabelAllTasks={
                  canAnnotate
                    ? () => {
