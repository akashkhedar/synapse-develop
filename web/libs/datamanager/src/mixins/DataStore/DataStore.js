  ...
  const model = types
    ...
    .actions((self) => ({
      ...
      _performFetch: flow(function* ({
        ...
        if (list) {
          // Hydrate missing users from annotators before storing tasks
          // This prevents MST reference resolution errors when annotators are external users
          list.forEach((task) => {
            task.annotators?.forEach((annotator) => {
              const userId = annotator.user_id;
              if (!userId) return;

              // Use action to add user to store (required by MST)
              root.addUserIfMissing({
                id: userId,
                email: annotator.email ?? "",
                username: annotator.username ?? `user-${userId}`,
                firstName: annotator.firstName ?? "",
                lastName: annotator.lastName ?? "",
                initials: "",
                lastActivity: "",
                avatar: annotator.avatar ?? null,
              });
            });
          });
          ...
          try {
            const ids = Array.from(self.list || [])
              .slice(0, 10)
              .map((i) => i.id);
            console.log(
              `[DataStore._performFetch] apiMethod=${apiMethod} total=${total} listLength=${
                self.list?.length ?? 0
              } sampleIds=`,
              ids
            );
            if (self.list?.length) {
              ...
              console.log(
                "[DataStore._performFetch] firstItemSnapshot:",
                first && (first.toJSON ? first.toJSON() : first)
              );
            }
          } catch (e) {
            console.error("[DataStore._performFetch] debug log failed:", e);
          }
        }
        ...
        // DEBUG: Log image paths from fetched tasks
        try {
          console.log("[DataStore] Tasks fetched:", {
            total: self.total,
            count: self.list?.length,
          });
          
          // Log first 3 tasks with their image data
          const sampleTasks = self.list.slice(0, 3);
          sampleTasks.forEach((task, idx) => {
            const imageValue = task.data?.get?.('image') || task.data?.image;
            console.log(`[DataStore] Task ${idx + 1} (ID: ${task.id}):`, {
              imageValue,
              imageType: typeof imageValue,
              dataKeys: task.data instanceof Map ? Array.from(task.data.keys()) : Object.keys(task.data || {}),
              fullImageURL: imageValue ? `${window.location.origin}${imageValue}` : 'NO IMAGE',
            });
          });
        } catch (e) {
          console.error("[DataStore] Image debug log failed:", e);
        }

        self.loading = false;

        root.SDK.invoke("dataFetched", self);
        // Log store summary after dataFetched event
        try {
          console.log("[DataStore._performFetch] store after dataFetched:", {
            apiMethod,
            total: self.total,
            listLength: self.list?.length ?? 0,
            firstId: self.list?.[0]?.id,
          });
        } catch (e) {
          console.error("[DataStore._performFetch] post fetch log error:", e);
        }
      }),
