...
export const AppStore = types
  ...
  .views((self) => ({
    ...
    /** @returns {import("../sdk/sf-sdk").SFWrapper} */
    get SF() {
      return self.SDK.sf;
    },
  ...
  .actions((self) => ({
    ...
    setTask: flow(function* ({ taskID, annotationID, pushState }) {
      ...
      // wait for the task to be loaded and SF to be initialized
      yield taskPromise.then(async () => {
        // wait for self.SF to be initialized with currentAnnotation
        let maxWait = 1000;
        while (!self.SF?.currentAnnotation && maxWait > 0) {
        ...
        if (self.sf) {
          ...
          self.SF?.setSFTask(self.taskStore.selected, id);

          if (isFF(FF_REGION_VISIBILITY_FROM_URL)) {
            const { annotation: annIDFromUrl, region: regionIDFromUrl } =
              History.getParams();
            const annotationStore = self.SF?.sf?.annotationStore;
            ...
            if (annIDFromUrl && annotationStore) {
              ...
              if (sfAnnotation) {
                ...
                self.SF?.setSFTask(
                  ...
                  annID,
                  undefined,
                  sfAnnotation.type === "prediction"
                );
              }
            }
            if (regionIDFromUrl) {
              const currentAnn = self.SF?.currentAnnotation;
          ...
          console.error("SF not initialized properly");
        }
    ...
    startLabelStream(options = {}) {
      ...
      if (
        isFF(FF_DEV_2887) &&
        self.SF?.sf?.annotationStore?.selected?.commentStore?.hasUnsaved
      ) {
        Modal.confirm({
    ...
    fetchUsers: flow(function* () {
      const result = yield self.apiCall("users");
