...
import { getAnnotationSnapshot } from "../../sdk/sf-utils";
...
export const create = (columns) => {
  const TaskModelBase = DynamicModel("TaskModelBase", columns, {
    data: types.optional(
      types.map(types.union(types.string, types.number, types.null)),
      {}
    ),
    ...(isFF(FF_DEV_2536)
    ...
    .actions((self) => ({
      loadTaskHistory: flow(function* (props) {
        ...
        if (!Array.isArray(taskHistory)) {
          console.warn("Invalid taskHistory response", taskHistory);
          return [];
        }
      ...
      loadNextTask: flow(function* ({ select = true } = {}) {
        ...
        if (isLabelStream) {
          ...
          console.log(
            `[LABEL STREAM] ${task.queue}, task ${task.id}, project ${
              getRoot(self)?.SDK?.project?.id
            }, user ${getRoot(self).SF.sf.user.id}${
