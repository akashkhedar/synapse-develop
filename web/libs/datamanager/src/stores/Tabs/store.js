  ...
  .actions((self) => ({
    setSelected: flow(function* (view, options = {}) {
      ...
      if (selected && self.selected !== selected) {
        ...
        try {
          ...
          // Log error and snapshot to help debugging MST creation issues, but
          // don't block UI — proceed without reload.
          try {
            // eslint-disable-next-line no-console
            console.error("Tabs.setSelected: selected.reload() failed", err, {
              selectedSnapshot: selected?.serialize
                ? selected.serialize()
                : selected,
            });
          } catch (e) {
        ...
        try {
          ...
          // eslint-disable-next-line no-console
          console.error("Tabs.setSelected: SDK.invoke tabChanged failed", err);
        }

        try {
          selected.selected._invokeChangeEvent();
        } catch (err) {
          // Log and continue — this often surfaces MST creation timing issues
          try {
            // eslint-disable-next-line no-console
            console.error(
              "Tabs.setSelected: selected.selected._invokeChangeEvent() failed",
              err,
              {
                selectedSnapshot: selected?.serialize
                  ? selected.serialize()
    ...
    createSnapshot(viewSnapshot = {}) {
      ...
      const urlTabIsVirtualCandidate = !!(
        viewSnapshot?.tab && isNaN(viewSnapshot.tab)
      );
    ...
    addView: flow(function* (viewSnapshot = {}, options) {
      ...
      // Defensive logging: log snapshot shape before creating MST node
      try {
        if (process && process.env && process.env.NODE_ENV === "development") {
          // shallow inspect keys and types to avoid huge output
          const inspect = Object.keys(newSnapshot).reduce((acc, k) => {
            acc[k] = Array.isArray(newSnapshot[k])
              ? `Array(${newSnapshot[k].length})`
              : typeof newSnapshot[k];
            return acc;
          }, {});
          // eslint-disable-next-line no-console
          console.debug("Tabs.addView: creating view from snapshot", inspect);
        }

        self.views.push(newSnapshot);
      } catch (err) {
        // Log full snapshot for investigation, then rethrow
        try {
          // eslint-disable-next-line no-console
          console.error(
            "Tabs.addView: failed to create view. snapshot:",
            newSnapshot
          );
        } catch (e) {
          // ignore
