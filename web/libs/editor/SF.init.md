...
Different thoughts and investingations related to SF init.
...
Main flow: Synapse -> constructor -> createApp -> configureStore -> initializeStore (+src/Component does the similar)
...
Should prevent some types of objects from requesting resources too early because the whole SF will be recreated.
...
For some reason it's set to `false` in sf-sdk constructor, but `true` in ReviewPage's setTask.
But is set to `true` in sf-sdk setSFTask.

Apparently it was not used anymore after https://github.com/Synapse/synapse-frontend/pull/1166
...
- draft is loaded without annotation in `setAnnotation()` in sf-sdk
...
- `setSFTask()`
- `onStorageInitialized()` — called from `AppStore#initializeStore()`, one of the core SF methods; if there is a task and that’s not a Label Stream it selects annotation based on `initialAnnotation` and `lastAnnotation`
- `onDeleteAnnotation()`
...
https://github.com/Synapse/synapse-frontend/pull/774. So they are removed along with anything related.
...
# Still left to fix after https://github.com/Synapse/synapse-frontend/pull/1640
...
https://github.com/Synapse/synapse-frontend/blob/d0dd212bf7b12cb7f4469c9c2aa32dfe166f5abe/src/stores/RegionStore.js#L181-L183
could be shifted towards self.selection.selectRegions(regions) and then
https://github.com/Synapse/synapse-frontend/blob/d0dd212bf7b12cb7f4469c9c2aa32dfe166f5abe/src/stores/RegionStore.js#L45-L47
...
- removed panels from App and down the stream — we don't use them anywhere (3rd party SF can be affected, but not sure anyone using panels there)
- converted `Synapse.js` to TypeScript
...
- removed `hydrated` as it's not used anymore after https://github.com/Synapse/synapse-frontend/pull/1166
