...
import { Synapse as SynapseReact } from "./Component";
...
// @todo type SFOptions = SnapshotIn<typeof AppStore>;
...
type SFOptions = Record<string, any> & {
  interfaces: string[];
  keymap?: any;
  user?: SFUser;
  users?: SFUser[];
...
export class Synapse {
  static Component = SynapseReact;

  static instances = new Set<Synapse>();

  static destroyAll() {
    Synapse.instances.forEach((inst) => inst.destroy?.());
    Synapse.instances.clear();
  }

  options: Partial<SFOptions>;
  root: Element | string;
  store: any;
  reactRoot: any;
  ...
  constructor(root: Element | string, userOptions: Partial<SFOptions> = {}) {
  ...
  // and we can update our other usages of Synapse to use createRoot, namely tests will likely be affected.
  async createAppV17() {
