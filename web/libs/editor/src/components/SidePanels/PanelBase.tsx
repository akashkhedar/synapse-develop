...
export const PanelBase: FC<PanelBaseProps> = ({
  ...
  const currentIcon = useMemo(() => {
    ...
    if (alignment === "left")
      return visible ? <IconArrowLeft /> : <IconArrowRight />;
    if (alignment === "right")
      return visible ? <IconArrowRight /> : <IconArrowLeft />;
    ...
    >
      <div className={cn("panel").elem("content").toClassName()}>
        {!locked && (
          ...
          >
            {(visible || detached) && (
              <div className={cn("panel").elem("title").toClassName()}>
                {title}
              </div>
            )}
