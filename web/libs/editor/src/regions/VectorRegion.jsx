 ...
 *
 * Handles conversion between Synapse's relative coordinates (0-100%) and KonvaVector's image coordinates (pixels).
 *
 * **Coordinate Systems:**
 * - **Synapse**: Relative coordinates (percentages) for storage and portability
 * - **KonvaVector**: Image coordinates (pixels) for precise editing
 *
 * **Conversion Flow:**
 * ```
 * Synapse (Relative %) ←→ VectorRegion ←→ KonvaVector (Image px)
 ...
 * - `serialize()`: Exports in Synapse format with relative coordinates
 */
const Model = types
  .model({
  ...
  .actions((self) => {
    return {
      /**
       * Converts coordinates from Synapse format (relative %) to KonvaVector format (image pixels)
      ...
      /**
       * Serializes region data in Synapse format with relative coordinates
       ...
       *
       * @return {VectorRegionResult} The serialized vector region data in Synapse format
       */
      serialize() {
