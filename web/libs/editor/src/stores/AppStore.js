...
export default types
  .model("AppStore", {
    ...
    /**
     * Flag
     * Description of task in Synapse
     */
    ...
    isSubmitting: false,
    /**
     * Flag for disable task in Synapse
     */
  ...
  .volatile(() => ({
    version: typeof SF_VERSION === "string" ? SF_VERSION : "0.0.0",
    initialized: false,
    hydrated: false,
  ...
  .actions((self) => {
    ...
    function initializeStore({ annotations = [], completions = [], predictions = [], annotationHistory }) {
      ...
      // Hydrate users from annotations BEFORE deserializing them
      // This ensures user references can be resolved properly
      [...completions, ...annotations].forEach((annotation) => {
        if (annotation.user && typeof annotation.user === 'object') {
          // User is a full object, add it to users store
          self.addUserIfMissing(annotation.user);
        } else if (annotation.completed_by && typeof annotation.completed_by === 'object') {
          // Fallback to completed_by if user is not present
          self.addUserIfMissing(annotation.completed_by);
        }
      });
      ...
      if (self.simpleInit) {
        ...
        console.log("sf: deserialization is finished");
    ...
    function addUserIfMissing(user) {
      ...
      const existingUser = self.users.find(u => u.id === user.id);
      if (!existingUser) {
