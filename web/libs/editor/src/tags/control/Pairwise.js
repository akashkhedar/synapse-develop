 ...
 * @meta_description Customize Synapse with the Pairwise tag for object comparison tasks for machine learning and data science projects.
  ...
  .actions((self) => ({
    ...
    setResult(dir = "none") {
      ...
      if (!self.left || !self.right) {
        ...
        console.warn(
          "Pairwise: left or right object reference is missing. Check toName and config.",
          {
            left: self.left,
            right: self.right,
          }
      ...
      const interactive =
        "cursor-pointer hover:bg-primary-emphasis-subtle transition-colors rounded-sm";
      if (self._selectionType === "className") {
        ...
        self.left.addProp(
          ...
          dir === "left"
            ? self._selection
            : { border: "1px solid transparent", borderRadius: "0.125rem" }
        );
        self.right.addProp(
          "style",
          dir === "right"
    ...
    afterCreate() {
      ...
      if (self.selectionstyle) {
        ...
        // See: synapse/web/libs/ui/src/tokens/tokens.js
        selection =
          "bg-primary-background border border-primary-border-subtle rounded-sm";
