...
examples.forEach((example) => {
  ...
  Scenario(`Different from_name -> ${title}`, async ({ I, Synapse, AtOutliner, AtAudioView }) => {
    ...
    if (regionsCount) {
      const restored = await Synapse.serialize();
  ...
  Scenario(`Nonexistent from_name -> ${title}`, async ({ I, Synapse, AtTopbar, AtOutliner }) => {
    const params = { annotations: [{ id: "test", result }], data };
    const configTree = Utils.parseXml(config);
...
[SINGLE_TYPE, MULTIPLE_TYPE].forEach((type) => {
  Scenario(`Making labels empty -> choice="${type}"`, async ({ I, Synapse, AtOutliner, AtAudioView, AtLabels }) => {
...
Scenario("Consistency of empty labels", async ({ I, Synapse, AtOutliner, AtImageView, AtLabels, AtPanels }) => {
