...
async function initSynapse({
  ...
  window.Synapse = new window.Synapse("synapse", { interfaces, config, task, settings, ...params });
}
...
const createSynapseInitFunction = (params) => {
...
function ${createMethodInjectionIntoScript("initSynapse", initSynapse)}
const fns = {${fns.join(",")}};
const params = ${JSON.stringify(preparedParams)};
initSynapse(linkFunctions(params));
`,
  );
};
 ...
 * Sets given feature flags before SF init
...
const createAddEventListenerScript = (eventName, callback) => {
  ...
  return new Function(
    ...
    window.Synapse.on("${eventName}",${eventName});
`,
  );
};
