...
Scenario("Image list with page navigation", async ({ I, AtImageView, Synapse }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [{ id: 1, result: [] }],
  ...
  const prevPageButton = locate(".sf-pagination__btn.sf-pagination__btn_arrow-left");
  const nextPageButton = locate(".sf-pagination__btn.sf-pagination__btn_arrow-right");
...
Scenario("Image list with hotkey navigation", async ({ I, AtImageView, Synapse }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [{ id: 1, result: [] }],
...
Scenario("View All disables MIG pagination", async ({ I, AtImageView, Synapse }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [
  ...
  const prevSelector = ".sf-pagination__btn_arrow-left";
  const nextSelector = ".sf-pagination__btn_arrow-right";
...
Scenario(
  ...
  async ({ I, AtImageView, Synapse }) => {
    ...
    await Synapse.resultsNotChanged(result);
  },
);

Scenario("Image list exports correct data", async ({ I, Synapse, AtImageView }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [{ id: 1, result }],
...
Scenario("Regions are not changes when duplicating an annotation", async ({ I, Synapse, AtImageView }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [{ id: 1, result }],
...
Scenario("No errors during brush export in MIG", async ({ I, Synapse, AtImageView, AtLabels, AtPanels }) => {
  const params = {
    config: brushConfig,
    data,
    annotations: [{ id: 1, result: [] }],
