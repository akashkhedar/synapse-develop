...
async function testRegion(testType, toolAccelerator, I, Synapse, AtImageView, AtOutliner) {
  const params = {
    config,
    data,
...
Scenario("Selected brush region should stay between tools", async ({ I, Synapse, AtImageView, AtOutliner }) => {
  await testRegion("brush", "B", I, Synapse, AtImageView, AtOutliner);
});

Scenario(
  "Selected Magic Wand region should stay between tools",
  async ({ I, Synapse, AtImageView, AtOutliner }) => {
    await testRegion("magicwand", "W", I, Synapse, AtImageView, AtOutliner);
  },
);
