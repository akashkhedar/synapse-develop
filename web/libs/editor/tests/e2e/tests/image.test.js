...
Scenario("Check Rect region for Image", async ({ I, Synapse, AtImageView, AtOutliner, AtPanels }) => {
  const params = {
    config,
    data: { image },
...
Scenario("Image with perRegion tags", async ({ I, Synapse, AtOutliner }) => {
  let result;
  const params = {
...
Data(outOfBoundsFFs).Scenario(
  ...
  async ({ I, AtLabels, AtOutliner, AtImageView, Synapse, AtPanels, current }) => {
