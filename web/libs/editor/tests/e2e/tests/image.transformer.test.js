...
Data(shapesTable)
  .Scenario(
    ...
    async ({ I, Synapse, AtImageView, AtOutliner, AtPanels, current }) => {
      ...
      Synapse.init(getParamsWithLabels(shapeName));
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMoveToolTransformer)).Scenario(
  ...
  async ({ I, Synapse, AtImageView, AtOutliner, AtPanels, current }) => {
    ...
    I.amOnPage("/");
    Synapse.init(getParamsWithShape(shapeName, Shape.params));
    AtDetailsPanel.collapsePanel();
    Synapse.waitForObjectsReady();
    AtOutliner.seeRegions(0);
    await AtImageView.lookForStage();
    ...
    const rectangleResult = await Synapse.serialize();
...
Data(shapesTable.filter(({ shapeName }) => shapeName === "Rectangle")).Scenario(
  ...
  async ({ I, Synapse, AtImageView, AtOutliner, AtPanels, current }) => {
    ...
    // Check resulting sizes
    rectangleResult = await Synapse.serialize();
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMoveToolTransformer)).Scenario(
  ...
  async ({ I, Synapse, AtImageView, AtOutliner, AtPanels, Regions, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
    const AtDetailsPanel = AtPanels.usePanel(AtPanels.PANEL.DETAILS);
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionRotator)).Scenario(
  ...
  async ({ I, Synapse, AtImageView, AtOutliner, AtPanels, current }) => {
...

    {
      ...
      let rectangleResult = await Synapse.serialize();
