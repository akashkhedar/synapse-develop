...
Scenario("NERText", async ({ I, Synapse, AtOutliner, AtTopbar }) => {
  const params = {
...
Scenario("NER Text with text field missing", async ({ I, Synapse }) => {
  const params = {
...
Scenario("NER Text from url", async ({ I, Synapse }) => {
  const params = {
...
Scenario("NER Text from url with text saved", async ({ I, Synapse }) => {
  const params = {
...
Scenario("NER Text with SECURE MODE and wrong valueType", async ({ I, Synapse }) => {
  const params = {
    annotations: [{ id: "TestCmpl", result: results }],
    config: configSimple,
    data: { url },
...
Scenario("NER Text with SECURE MODE", async ({ I, Synapse }) => {
  const params = {
    annotations: [{ id: "TestCmpl", result: resultsFromUrl }],
    config: configUrl,
    data: { url },
  ...
  I.waitForElement(".sf-richtext__line", 60);
...
Scenario("NER Text regions in Outliner", async ({ I, Synapse }) => {
  const params = {
    annotations: [{ id: "TestCmpl", result: resultsFromUrl }],
    config: configUrl,
    data: { url },
  ...
  I.seeElement(locate(".sf-outliner-item").withText(resultsFromUrl[0].value.text));
  I.seeElement(locate(".sf-outliner-item").withText(resultsFromUrl[1].value.text));
});
