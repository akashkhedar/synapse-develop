...
Scenario("Check simple nested Choices for Text", async ({ I, Synapse }) => {
  const params = {
    config: configSimple,
    data: { reviewText },
  };
...
Scenario("Check good nested Choice for Text", async ({ I, Synapse, AtLabels, AtOutliner }) => {
  const params = {
...
Scenario("Check removing unexpected results based on visibleWhen parameter", async ({ I, Synapse }) => {
  const params = {
    config: `<View>
  <Text name="text" value="$reviewText" valueType="text" />
  <Choices name="sentiment" toName="text" showInLine="true">
