...
Scenario("Basic scenario", async ({ I, Synapse, AtImageView, AtSettings, AtLabels, AtOutliner }) => {
  I.amOnPage("/");
  ...
  I.pressKey("Enter");
  const results = await Synapse.serialize();
...
Scenario(
  ...
  async ({ I, Synapse, AtImageView, AtSettings, AtLabels, AtOutliner }) => {
    I.amOnPage("/");
    Synapse.init({ config: createConfig(), data });
    Synapse.waitForObjectsReady();
    AtSettings.open();
    AtSettings.setGeneralSettings({
      [AtSettings.GENERAL_SETTINGS.SHOW_LABELS]: true,
    ...
    for (const region of Object.values(regions)) {
      ...
      if (region.text) {
        I.fillField(AtOutliner.locateSelectedItem().find(".sf-textarea-tag__input"), region.text);
      }
    }
    const results = await Synapse.serialize();
    ...
    session("Deserialization", () => {
      I.amOnPage("/");
      Synapse.init({ config: createConfig(), data, annotations: [{ id: "test", result: results }] });
      Synapse.waitForObjectsReady();
      AtOutliner.seeRegions(regions.length);
      for (const [idx, region] of Object.entries(regions)) {
        if (region.text) {
          I.seeInField(AtOutliner.locateRegionIndex(+idx + 1).find(".sf-textarea-tag__input"), region.text);
        }
      }
    });
  },
