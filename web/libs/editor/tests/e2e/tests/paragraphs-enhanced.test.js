...
Scenario(
  ...
  async ({ I, Synapse, AtOutliner, AtParagraphs, AtLabels }) => {
    await retryScenario(async () => {
      const params = { data: DATA, config: CONFIG };
      I.amOnPage("/");
      Synapse.setFeatureFlags(FEATURE_FLAGS);
      Synapse.init(params);
      AtOutliner.seeRegions(0);

      // Debug: Wait for any label to appear and print all label texts
      I.waitForElement(".sf-label", 10);
      const labelTexts = await I.grabTextFromAll(".sf-label");
      I.say(`Visible labels: ${JSON.stringify(labelTexts)}`);

      // Wait for the specific label to appear
      I.waitForElement(locate(".sf-label").withText("General: Positive1"), 10); // Increased timeout
...
Scenario("Select All button is disabled when no label is selected", async ({ I, Synapse, AtOutliner }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Synapse.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey for Select All creates region", async ({ I, Synapse, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Synapse.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Next Phrase moves focus to next phrase", async ({ I, Synapse, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Synapse.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Previous Phrase moves focus to previous phrase", async ({ I, Synapse, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Synapse.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Select All and Annotate creates region", async ({ I, Synapse, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Synapse.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Next Region in Phrase navigates to next region", async ({ I, Synapse, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Synapse.setFeatureFlags(FEATURE_FLAGS);
...
Scenario(
  ...
  async ({ I, Synapse, AtOutliner, AtLabels }) => {
    await retryScenario(async () => {
      const params = { data: DATA, config: CONFIG };
      I.amOnPage("/");
      Synapse.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Next/Previous Region loops at ends", async ({ I, Synapse, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Synapse.setFeatureFlags(FEATURE_FLAGS);
    ...
    I.click(locate(".sf-outliner-item").at(3));
    ...
    const selectedIdxAfterNext = await I.executeScript(() => {
      const items = Array.from(document.querySelectorAll(".sf-outliner-item"));
    ...
    I.click(locate(".sf-outliner-item").at(1));
