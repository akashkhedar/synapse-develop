...
Scenario(
  ...
  async ({ I, Synapse, AtOutliner, AtParagraphs, AtLabels }) => {
    ...
    const twoActionsResult = await Synapse.serialize();
    ...
    const oneActionResult = await Synapse.serialize();
...
Scenario("Check different cases ", async ({ I, Synapse, AtOutliner, AtParagraphs, AtLabels }) => {
  const dialogue = [
    1, // 1
    3, // 2
    1, // 3
...
Scenario(
  ...
  async ({ I, Synapse, AtOutliner, AtParagraphs, AtLabels }) => {
    ...
    if (filterButtonVisible === 0) {
      I.say("Filter button not found, reinitializing Synapse");
      Synapse.init(params);
      I.wait(1);
    }
