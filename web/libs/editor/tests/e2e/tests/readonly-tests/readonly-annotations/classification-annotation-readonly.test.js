...
Data(imageExamples).Scenario("Classification Readonly Annotations", async ({ I, current, Synapse }) => {
  ...
  I.seeElement(".sf-number input:disabled");
  ...
  I.dontSee('.sf-text-area [aria-label="Edit Region"]');

  I.say("Check region is not deletable");
  I.dontSee('.sf-text-area [aria-label="Delete Region"]');

  /**
   * Checking the Choices
   */
  I.say("Checking the Choices");
  I.seeElement(".sf-choices input:disabled");

  I.say("Try selecting anyways");
  I.checkOption("Choice 1", ".sf-choices");
