...
Scenario("Annotation button should keep border width on hover", async ({ I, Synapse }) => {
  ...
  const borderWidth = await I.executeScript(() => {
    const el = document.querySelector(".sf-annotation-button:nth-child(2)");
    const borderWidth = window.getComputedStyle(el).getPropertyValue("border-width");

    return borderWidth;
  });

  I.moveCursorTo(".sf-annotation-button:nth-child(2)");
