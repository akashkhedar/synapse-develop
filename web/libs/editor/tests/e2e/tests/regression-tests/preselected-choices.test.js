...
Scenario("Make a duplicate of annotation with preselected choices", async ({ I, Synapse, AtTopbar }) => {
  const params = {
    config: `
<View>
  ...
  const duplicateResult = await Synapse.serialize();
  ...
  const annotationWithPresetValues = await Synapse.serialize();
...
Scenario("Make a duplicate of empty annotation with preselected choices", async ({ I, Synapse, AtTopbar }) => {
  const params = {
    config: `
<View>
  <Text name="text" value="$text"></Text>
