...
const checkScrollToSelectedPersists = (I, label, outliner) => {
  I.click(locate(`.sf-${outliner ? "outliner" : "region"}-item__title *`).withText(label));
  I.waitForVisible(locate(".sf-label_selected").withText(label));
};

const checkPaginateToSelectedPersists = (I, label, page, outliner) => {
  checkScrollToSelectedPersists(I, label, outliner);
  I.seeElement(locate(".sf-pagination__page-indicator").withText(`${page}`));
};

const checkPaginationButtons = (I) => {
  I.click(locate(".sf-pagination__btn_arrow-left-double"));
  I.seeElement(locate(".sf-pagination__page-indicator").withText("1"));

  I.click(locate(".sf-pagination__btn_arrow-right"));
  I.seeElement(locate(".sf-pagination__page-indicator").withText("2"));

  I.click(locate(".sf-pagination__btn_arrow-right"));
  I.seeElement(locate(".sf-pagination__page-indicator").withText("3"));
};
...
Scenario("Outliner Regions will paginate view window on region click and page advance", async ({ I, Synapse }) => {
...
Scenario("Outliner Regions will scroll view window on region click", async ({ I, Synapse }) => {
