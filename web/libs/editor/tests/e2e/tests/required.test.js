...
Scenario("Check required param", async ({ I, Synapse, Modals }) => {
...
Scenario("Check required param in complex config", async ({ I, Synapse, AtOutliner, Modals }) => {
...
Scenario("Check required param with visibleWhen='choice-unselected'", async ({ I, Synapse, Modals }) => {
  ...
  Synapse.init(params);
  // Synapse is reloaded, there are no new annotation from prev steps
  I.dontSee("New annotation");
  I.click('[aria-label="Annotations List Toggle"]');
  I.click('[aria-label="Create Annotation"]');
