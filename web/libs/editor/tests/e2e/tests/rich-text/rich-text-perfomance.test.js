...
Scenario("Rich text initialization in hightload conditions", async ({ I, Synapse, AtOutliner }) => {
  ...
  I.executeScript(async () => {
    new Promise((resolve) => {
      const watchSynapseReady = () => {
        const isReady = window.document.querySelector(".sf-richtext");

        if (isReady) {
          window._startTime = Date.now();
          resolve(true);
        } else {
          setTimeout(watchSynapseReady, 100);
        }
      };
