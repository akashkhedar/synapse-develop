...
Data(configParams).Scenario(
  ...
  async ({ I, Synapse, AtOutliner, current }) => {
...
Data(configParams).Scenario("Should work with emoji.", async ({ I, Synapse, AtOutliner, current }) => {
  const { inline } = current;
  const config = createConfig({
    rows: inline ? "1" : "3",
  });
...
Data(configParams).Scenario("Should work with existent regions.", async ({ I, Synapse, AtOutliner, current }) => {
  const { inline } = current;
  const config = createConfig({
    rows: inline ? "1" : "3",
  });
  ...
  const initialResult = await Synapse.serialize();
...
`;
  ...
  Data(paramsTable).Scenario("Initial focus", async ({ I, Synapse, AtOutliner, current }) => {
