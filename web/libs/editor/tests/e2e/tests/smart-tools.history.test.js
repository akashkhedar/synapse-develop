...
function getRectangleSuggestions(reg, group) {
  ...
  const annotation = window.Synapse.store.annotationStore.selected;
  ...
  window.Synapse.store.loadSuggestions(new Promise((resolve) => resolve(suggestions)), (x) => x);
}

Scenario("Undo regions auto-annotated from predictions", async ({ I, Synapse, AtImageView, AtOutliner }) => {
  I.amOnPage("/");
  Synapse.init({
...
Scenario(
  ...
  async ({ I, Synapse, AtImageView, AtOutliner, AtPanels }) => {
