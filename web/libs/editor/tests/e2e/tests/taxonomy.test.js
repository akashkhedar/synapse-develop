...
Scenario("Lines overlap", async ({ I, Synapse, AtTaxonomy }) => {
...
Scenario("Add custom items", async ({ I, Synapse, AtTaxonomy }) => {
  const params = {
    config: `
<View>
  <Text name="text" value="$text"/>
  ...
  const userLabels = await Synapse.grabUserLabels();
  ...
  await session("Deserialization", async () => {
    ...
    Synapse.initUserLabels(userLabels);
    ...
    const result2 = await Synapse.serialize();
  ...
  const userLabels2 = await Synapse.grabUserLabels();
...
Scenario("Non unique values filtering", async ({ I, Synapse, AtTaxonomy }) => {
  I.amOnPage("/");
  Synapse.init({
    config: `
<View>
 ...
 */
Scenario.skip("Taxonomy read only in history", async ({ I, Synapse, AtTaxonomy }) => {
...
Scenario("Taxonomy readonly result", async ({ I, Synapse, AtTaxonomy }) => {
  I.amOnPage("/");
  Synapse.init({
    config: `
<View>
...
Scenario("Taxonomy per region", async ({ I, Synapse, AtTaxonomy, AtOutliner }) => {
  const config = `
<View>
  <Text name="text" value="$text"/>
...
Scenario("Aliases in Taxonomy", async ({ I, Synapse, AtTaxonomy }) => {
...
Scenario("Taxonomy dynamic items", async ({ I, Synapse, AtTaxonomy }) => {
  const data = {
    text: "A text",
    options: [
      {
...
Scenario("Taxonomy maxUsages", async ({ I, Synapse, AtTaxonomy }) => {
  const config = `
<View>
  <Text name="text" value="$text"/>
...
Scenario("Taxonomy visibleWhen", async ({ I, Synapse, AtTaxonomy }) => {
...
Scenario("Taxonomy visibleWhen with aliases", async ({ I, Synapse, AtTaxonomy }) => {
  const createConfig = ({ showFullPath = false, visibleWhen = "choice-selected", whenChoiceValue = "Four" } = {}) => `
<View>
  <Text name="text" value="$text"/>
  <Taxonomy required="true" name="taxonomy" toName="text" leafsOnly="true" placeholder="Select something..." showFullPath="${showFullPath}">
