...
const scenarios = {
  "Works with data sorted ascending by time column": {
    ...
    assert(I) {
      ...
      I.dontSeeElement(locate(".sf-errors"));
    },
  },
...
Object.entries(scenarios).forEach(([title, scenario]) =>
  Scenario(title, async ({ I, Synapse }) => {
...
Scenario("TimeSeries with optimized data", async ({ I, Synapse, AtTimeSeries }) => {
