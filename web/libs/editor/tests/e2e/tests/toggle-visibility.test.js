...
const ONE_VISIBLE_SELECTOR = ".sf-tree__node:not(.sf-tree__node_type_label):not(.sf-tree__node_hidden)";
const ONE_HIDDEN_SELECTOR = ".sf-tree__node:not(.sf-tree__node_type_label).sf-tree__node_hidden";
const ONE_VISIBILITY_TOGGLE_BUTTON = ".sf-outliner-item__control_type_visibility button";
...
Scenario("Checking mass toggling of visibility", async ({ I, Synapse, AtOutliner }) => {
  ...
  await I.amOnPage("/");
  Synapse.init({ annotations, config, data });
...
Scenario("Hiding bulk visibility toggle", ({ I, Synapse, AtImageView, AtLabels, AtOutliner }) => {
  I.amOnPage("/");
  Synapse.init({ config, data });
  Synapse.waitForObjectsReady();
...
Scenario("Checking regions grouped by label", async ({ I, Synapse }) => {
  const checkVisible = async (num) => {
    switch (num) {
      case 0:
        I.seeElement(ALL_HIDDEN_SELECTOR);
  ...
  I.waitForElement(".sf-view-controls", 5);

  // Switch to label grouping using UI interaction
  I.click('[data-testid="grouping-manual"]');
  I.waitForElement(".sf-menu", 2);

  I.waitForElement(".sf-dropdown.sf-visible");
  // Click on the "Group by Label" option in the dropdown
  I.click('//div[contains(@class, "sf-view-controls__label") and contains(text(), "Group by Label")]');
