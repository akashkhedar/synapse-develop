...
for (const isFFDev3873 of [true, false]) {
  ...
  describe(`Bulk mode ${ffModePostfix}`, () => {
    ...
    it("should not show per-region classifications", () => {
      ...
      cy.get(".sf-main-view__annotation div:eq(0)").should("be.empty");
    });
    ...
    it("should hide panels and buttons not related to the bulk mode", () => {
      Synapse.params().config("<View></View>").withInterface("annotation:bulk").data({}).withResult([]).init();

      cy.get(".sf-sidepanels").should("not.exist");
      cy.get(".sf-sidepanels__wrapper").should("not.exist");

      cy.get(".sf-current-task").should("not.exist");
      cy.get(`[aria-label="Compare all annotations"]`).should("not.exist");
      cy.get(`[aria-label="Create an annotation"]`).should("not.exist");
      cy.get(".sf-annotations-carousel").should("not.exist");
      cy.get(".sf-ground-truth").should("not.exist");
    });
  });
}
