import { Synapse, Sidebar, useTaxonomy } from "@synapse/frontend-test/helpers/SF";
...
describe("Control Tags - Text with Dual Taxonomy", () => {
  it("should select options from two taxonomies and create text region", () => {
    ...
    cy.log("Initialize Synapse with text and dual taxonomy configuration");
    Synapse.params().config(textWithDualTaxonomyConfig).data(simpleTextData).withResult([]).init();
    ...
    Synapse.serialize().then((results: any[]) => {
