import { useChoices, Synapse, Textarea, ImageView, Sidebar, Choices } from "@synapse/frontend-test/helpers/SF";
import {
...
const checkVisibility = (visibleIndexes: number[], totalIndexes: number) => {
  Array.from({ length: totalIndexes }).forEach((_, i) => {
    cy.get(`.sf-choices:eq(${i})`).should(visibleIndexes.includes(i) ? "be.visible" : "not.be.visible");
  });
};

describe("Visibility", () => {
  it("Ensure correct visibility of conditionally selected choices", () => {
    Synapse.params().config(simpleVisibleWhenVisibilityConfig).data(visibilityTextData).withResult([]).init();
  ...
  it("Ensure correct visibility of conditionally selected choices with parent visibility restrictions", () => {
    Synapse.params().config(multipleChainedViewsVisibilityConfig).data(visibilityTextData).withResult([]).init();
  ...
  it("Ensure correct visibility of conditionally selected and submitted textarea", () => {
    Synapse.params().config(textareaVisibilityConfig).data(visibilityTextData).withResult([]).init();
