...
describe("Image - Initial tool", () => {
  it("should be set from the first avaliable tool in the config - rectangle", () => {
    Synapse.params().config(imageWithRectanglesConfig).data(imageData).withResult([]).init();
  ...
  it("should be set from the first avaliable tool in the config - brush", () => {
    Synapse.params().config(imageWithBrushConfig).data(imageData).withResult([]).init();
    Synapse.waitForObjectsReady();
  ...
  it("should select first tool as default", () => {
    Synapse.params().config(imageWithBothToolsConfig).data(imageData).withResult([]).init();
    Synapse.waitForObjectsReady();
