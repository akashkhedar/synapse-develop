import { Labels, Synapse } from "@synapse/frontend-test/helpers/SF";
...
describe("NER - Emoji - Text", () => {
  ...
  it("Should calculate offsets by code points in text", () => {
    Synapse.params().config(simpleTextConfig).data(simpleTextData).withResult([]).init();
    ...
    Synapse.serialize().then((results) => {
      ...
      Synapse.params().config(simpleTextConfig).data(simpleTextData).withResult(results).init();
      Synapse.waitForObjectsReady();
      RichText.hasRegionWithText("test");
  ...
  it("Should calculate offsets by code points in multiline text", () => {
    Synapse.params().config(simpleTextConfig).data(multilineTextData).withResult([]).init();
    Synapse.waitForObjectsReady();
    Labels.select("region");
    ...
    Synapse.serialize().then((results) => {
      ...
      Synapse.params().config(simpleTextConfig).data(multilineTextData).withResult(results).init();
      Synapse.waitForObjectsReady();
      RichText.hasRegionWithText("Warning:");
      RichText.hasRegionWithText("🐱 This is a test");
  ...
  it("Should calculate global offsets by code points and relative offsets by string length in hypertext", () => {
    Synapse.params().config(simpleHyperTextConfig).data(simpleHyperTextData).withResult([]).init();
    Synapse.waitForObjectsReady();
    Labels.select("region");
    RichText.selectText("test");
    RichText.hasRegionWithText("test");
    Synapse.serialize().then((results) => {
      ...
      Synapse.params().config(simpleHyperTextConfig).data(simpleHyperTextData).withResult(results).init();
      Synapse.waitForObjectsReady();
      RichText.hasRegionWithText("test");
  ...
  it("Heuristic edge case", () => {
    Synapse.params().config(simpleHyperTextConfig).data({ text: "<p>🐱\nmeans cat</p>" }).withResult([]).init();
    Synapse.waitForObjectsReady();
    Labels.select("region");
