import { AudioView, Synapse, Paragraphs } from "@synapse/frontend-test/helpers/SF";
import { Network } from "@synapse/frontend-test/helpers/utils";
...
describe("Sync buffering playback", () => {
  ...
  it.skip("should go though all paragraphs during playback with buffering", () => {
    let attempts = 3;
    const testScenario = () => {
      Synapse.params().config(videoAudioParagraphsConfig).data(fullOpossumSnowData).withResult([]).init();
