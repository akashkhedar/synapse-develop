import { Labels, Synapse, Sidebar, VideoView } from "@synapse/frontend-test/helpers/SF/index";
...
describe("Video segmentation", suiteConfig, () => {
  it("Should be able to draw a simple rectangle", () => {
    Synapse.params().config(simpleVideoConfig).data(simpleVideoData).withResult([]).init();
  ...
  it("Should be invisible out of the lifespan (rectangle)", () => {
    Synapse.params().config(simpleVideoConfig).data(simpleVideoData).withResult(simpleVideoResult).init();
    Synapse.waitForObjectsReady();
    // Wait for video and regions to be fully loaded
