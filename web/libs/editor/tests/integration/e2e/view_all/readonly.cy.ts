import { AudioView, Choices, Synapse, Rating, Taxonomy, ToolBar } from "@synapse/frontend-test/helpers/SF";
...
describe("View all - Raadonly", () => {
  it("Should not allow user to edit an annotation - Rating", () => {
    Synapse.params().config(ratingConfig).data(textData).withResult(ratingResult).init();
    ToolBar.viewAllBtn.click();
    Rating.setValue(5);
    Rating.hasValue(3);
  });
  it("Should not allow user to edit an annotation - Taxonomy", () => {
    Synapse.params().config(taxonomyConfig).data(textData).withResult(taxonomyResult).init();
    ToolBar.viewAllBtn.click();
    Taxonomy.open();
    Taxonomy.dropdown.should("not.exist");
  });
  it("Should not allow user to edit an annotation - Audio region", () => {
    Synapse.params().config(audioConfig).data(audioData).withResult(audioResult).init();
    AudioView.isReady();
    ToolBar.viewAllBtn.click();
    AudioView.isReady();
  ...
  it("Should allow to select an audio region", () => {
    Synapse.params().config(audioPerRegionConfig).data(audioData).withResult(audioResult).init();
    AudioView.isReady();
    ToolBar.viewAllBtn.click();
    AudioView.isReady();
