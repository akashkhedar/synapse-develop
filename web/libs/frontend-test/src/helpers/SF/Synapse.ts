import { fixSFParams } from "@synapse/frontend-test/helpers/utils/fixSFParams";
...
class LSParamsBuilder {
  ...
  private ls: typeof Synapse = null;

  constructor(ls: typeof Synapse) {
    this.ls = ls;
  }
...
export const Synapse = {
  ...
  init(params: LSParams, beforeLoadCallback?: (win: Cypress.AUTWindow) => void) {
    ...
    cy.wrap(null).then(() => {
      ...
      cy.visit("/").then((win) => {
        ...
        const Synapse = new win.Synapse("synapse", fixSFParams(win.SF_CONFIG, win));

        if (win.SF_CONFIG.eventListeners) {
          for (const [event, listener] of Object.entries(win.SF_CONFIG.eventListeners)) {
            Synapse.on(event, listener);
          }
        }
        expect(win.Synapse.instances.size).to.be.equal(1);
        cy.get(".sf-editor").should("be.visible");
        cy.log("Synapse initialized");
      });
    });
  },
  ...
  /**
   * Exports current result from Synapse's selected annotationStore
   */
  serialize() {
   ...
   * It uses inner logic of Synapse's object tag models
   */
  waitForObjectsReady() {
    cy.window().then((win) => {
