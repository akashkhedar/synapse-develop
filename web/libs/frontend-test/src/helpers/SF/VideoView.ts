...
class VideoViewHelper extends withMedia(
  class implements ViewWithMedia {
    ...
    get videoCanvas() {
      return this.root.get(".sf-video-canvas");
    }

    get framesControl() {
      return this.root.find(".sf-frames-control");
    }

    get timelineContainer() {
      return this.root.get(".sf-video-segmentation__timeline");
    }

    get timelineToolbar() {
      return this.root.get(".sf-timeline__topbar");
    }

    get timeLine() {
      return this.timelineToolbar.find(".sf-seeker");
    }
    get frameCounter() {
      return this.timelineToolbar.get(".sf-frames-control");
    }
    ...
    get timeLineLabels() {
      return this.root.get(".sf-timeline-frames__labels-bg");
    }

    get timeframesArea() {
      return this.root.get(".sf-timeline-frames__scroll");
    }
    ...
    drawRect(x: number, y: number, width: number, height: number, options: MouseInteractionOptions = {}) {
      ...
      this.drawingArea
        ...
        // This problem could be solved in VideoRegions component of sf
    ...
    captureCanvas(name: string) {
      return this.drawingArea.captureScreenshot(name, { withHidden: [".sf-video-canvas"] });
    }
    ...
    canvasShouldChange(name: string, threshold = 0.1) {
      return this.drawingArea.compareScreenshot(name, "shouldChange", { withHidden: [".sf-video-canvas"], threshold });
    }
