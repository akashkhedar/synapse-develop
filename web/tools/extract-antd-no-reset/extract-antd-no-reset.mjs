...
let targetDirPath = path.join(
  nodeModulesPath,
  "@synapse/editor/src/assets/styles"
);
if (!fs.existsSync(targetDirPath)) {
  if (
    fs.existsSync(path.join(__dirname, "../../libs/editor/src/assets/styles"))
  ) {
...
const contentWithoutStylelintComments = contentAfterHeader.replace(
  /\/\* stylelint-disable[\s\S]*?\*\//g,
  ""
);

// Combine our header with the cleaned content
modifiedContent =
  modifiedContent.substring(0, headerEndPos) + contentWithoutStylelintComments;
